...
!
!    Local, integer ( kind = 4 ) N, the number of intervals in the spatial
!    domain.
!
...
!    current time.
!
!    Local, real ( kind = 8 ) U2(N+1,P+1), the PCE representation for the
!    next time.
!
...
!
        ...
        do j = 1, p + 1
          ...
          term1 = term1 + visc(i) &
            * ( u1(ix+1,j) - 2.0D+00 * u1(ix,j) + u1(ix-1,j) ) * tp / dp
        end do
!
!  Convective term.
!
        ...
        do j = 1, p + 1
          do i = 1, p + 1
            tp = he_triple_product_integral ( i - 1, j - 1, k - 1 )
            term2 = term2 &
              + ( conv * u1(ix,i) * ( u1(ix+1,j) - u1(ix-1,j) ) * tp ) / dp
          end do
        end do
...
!
!    Output, real ( kind = 8 ) HE_TRIPLE_PRODUCT_INTEGRAL, the value of 
!    the integral.
!
  implicit none
