...
!
  ...
  integer ( kind = 4 ), allocatable :: histo(:)
  real ( kind = 4 ) histo_ave
  integer ( kind = 4 ) histo_max
  integer ( kind = 4 ) histo_min
  real ( kind = 8 )  histo_std
  integer ( kind = 4 ) i
  integer ( kind = 4 ) iarg
...
!
  histo_ave = real ( sum ( histo(1:sub_num) ), kind = 8 ) &
    / real ( sub_num, kind = 8 )
...
!
  ...
  std = 0.0D+00
  do i = 1, n
    std = std + ( real ( a(i), kind = 8 ) - mean )**2
  end do
