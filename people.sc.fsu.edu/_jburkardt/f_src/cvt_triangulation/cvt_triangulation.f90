...
!
  ...
  if ( .false. ) then
    ...
    write ( *, '(a)' ) '  Skipping test05()'
  end if

  if ( .true. ) then
    call test06 ( )
  else
    write ( *, '(a)' ) '  Skipping test06()'
  end if
!
!  Terminate.
!
...
!
  ...
  real ( kind = 8 ) energy
  character ( len = 255 ) file_eps_name
  character ( len = 255 ) file_txt_name
...
!
  ...
  integer ( kind = 4 ) n
  integer ( kind = 4 ), parameter :: batch = 100
...
!    Input, integer ( kind = 4 ) N, the number of points.
!
!    Input, integer ( kind = 4 ) P(DIM_NUM,N), the point coordinates.
!
