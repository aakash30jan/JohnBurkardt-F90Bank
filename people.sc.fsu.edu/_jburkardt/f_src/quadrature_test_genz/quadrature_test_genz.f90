...
!
  ...
  real ( kind = 8 ) q
  real ( kind = 8 ) quad_err
  real ( kind = 8 ) quad_est
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'QUADRATURE_TEST_GENZ'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  This program is related to QUADRATURE TEST, but'
  write ( *, '(a)' ) '  only checks the Genz integrals.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the accuracy of a multidimensional'
  write ( *, '(a)' ) '  quadrature rule by integrating test functions.'
!
...
!
  ...
  if ( dim_num2 /= 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'QUADRATURE_TEST_GENZ - Fatal error!'
...
!
  ...
  if ( dim_num2 /= dim_num ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'QUADRATURE_TEST_GENZ - Fatal error!'
    write ( *, '(a)' ) '  The quadrature region file should have the same'
    write ( *, '(a)' ) '  number of values on each line as the abscissa file'
    write ( *, '(a)' ) '  does.'
    stop
  end if
...
!
  volume1 = abs ( product ( r(1:dim_num,2) - r(1:dim_num,1) ) )
  ...
  allocate ( a(dim_num) )
  allocate ( b(dim_num) )
  allocate ( f(point_num) )
  allocate ( x2(dim_num,point_num) )
!
!  In QUADRATURE TEST, problems 27 through 32 are the Genz integrals.
!
  do problem = 27, 32
!
...
!   if ( region(1:3) == 'box' .or. region(1:3) == 'BOX' ) then
      ...
      call p00_exact ( problem, dim_num, integral )

      call p00_lim ( problem, dim_num, a, b )

      call p00_volume ( problem, dim_num, volume2 )
!
...
!
      call p00_f ( problem, dim_num, point_num, x2, f )
!
!  Compute the weighted estimate.
!
!  NOTE: I was getting NaN for the result of QUAD_EST, inexplicably.
!  I had to artificially check for bad values and do the dot product by hand.
!
      if ( .true. ) then

        quad_est = dot_product ( w(1:point_num), f(1:point_num) ) &
          * volume2 / volume1
      else

        quad_est = 0.0D+00
        do j = 1, point_num
          if ( isnan ( f(j) * w(j) ) ) then
          else
            quad_est = quad_est + f(j) * w(j)
          end if
        end do
        quad_est = quad_est * volume2 / volume1

      end if

!     if ( integral == huge ( integral ) ) then

!       write ( *, '(2x,i4,2x,i4,2x,i10,2x,g14.6,2x,a,2x,a)' ) &
