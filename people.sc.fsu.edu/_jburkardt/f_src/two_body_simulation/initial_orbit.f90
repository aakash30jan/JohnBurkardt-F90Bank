...
!*****************************************************************************80
!
!! MAIN is the main program for INITIAL_ORBIT.
!
!  Discussion:
!
!    INITIAL_ORBIT uses RKF45 as an integrator for the a version
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INITIAL_ORBIT'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  This simulation follows a small body for two orbits'
  write ( *, '(a)' ) '  around a relatively massive body - such as Mercury around'
  write ( *, '(a)' ) '  the sun.'
  write ( *, '(a)' ) '  Kepler''s equations for a two body system are used.'
  write ( *, '(a)' ) '  Note that the orbit is NOT an ellipse.  But that''s OK,'
  write ( *, '(a)' ) '  because the planet is far from its equilibrium orbit.'
  write ( *, '(a)' ) ' '
...
!
  ...
  data_filename = 'initial_orbit_data.txt'
  ...
  command_filename = 'initial_orbit_commands.txt'
  ...
  write ( command_unit, '(a)' ) 'set output "initial_orbit.png"'
  write ( command_unit, '(a)' ) 'set xlabel "X"'
  write ( command_unit, '(a)' ) 'set ylabel "Y"'
  write ( command_unit, '(a)' ) 'set title "Initial orbit"'
...
!
  ...
  t_start = 0.0D+00
  t_stop = 2.0D+00 * 3.895D+00

  t = 0.0D+00
  t_out = 0.0D+00

  y(1:neqn) = (/ 1.0D+00, 0.0D+00,  0.0D+00,  0.8D+00 /)
