...
!    can also be stored in an integer ( kind = 4 ) I, with
!
!      I = AN*Q^N + ... + A0.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) IUNIT, the unit number to which the data
!    should be written.
!
!    Input, integer ( kind = 4 ) Q, the order of the field for which the
...
!
  ...
  integer ( kind = 4 ) gfadd(0:q_max-1,0:q_max-1)
  integer ( kind = 4 ) gfmul(0:q_max-1,0:q_max-1)
  integer ( kind = 4 ) i
  integer ( kind = 4 ) i4_characteristic
  integer ( kind = 4 ) irrply(8,-2:deg_max)
  integer ( kind = 4 ) iunit
  integer ( kind = 4 ) j
  integer ( kind = 4 ) modply(-1:deg_max)
...
!
!  Now deal with the rest.  Each integer ( kind = 4 ) from 1 to QIN-1
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) IN, the (nonnegative) integer containing the 
!    polynomial information.
!
!    Input, integer ( kind = 4 ) P, the order of the field.
!
!    Output, integer ( kind = 4 ) POLY(-1:deg_max), the polynomial information.
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) PA(-1:deg_max), the first polynomial.
!
!    Input, integer ( kind = 4 ) PB(-1:deg_max), the second polynomial.
!
!    Output, integer ( kind = 4 ) PC(-1:deg_max), the sum polynomial.
!
  implicit none
...
!    Input, integer ( kind = 4 ) PB(-1:deg_max), the second polynomial.
!
!    Output, integer ( kind = 4 ) PQ(-1:deg_max), the quotient polynomial.
!
!    Output, integer ( kind = 4 ) PR(-1:deg_max), the remainder polynomial.
!
  implicit none
  ...
  integer ( kind = 4 ) pa(-1:deg_max)
  integer ( kind = 4 ) pb(-1:deg_max)
  integer ( kind = 4 ) pq(-1:deg_max)
  integer ( kind = 4 ) pr(-1:deg_max)
  integer ( kind = 4 ) ptq(-1:deg_max)
  integer ( kind = 4 ) ptr(-1:deg_max)
  integer ( kind = 4 ) q
  integer ( kind = 4 ) sub(0:q_max-1,0:q_max-1)
...
!    Input, integer ( kind = 4 ) PB(-1:deg_max), the second polynomial.
!
!    Output, integer ( kind = 4 ) PC(-1:deg_max), the product polynomial.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) POLY(-1:deg_max), the polynomial information.
!    POLY(-1) contains the degree of the polynomial.  POLY(I) contains
!    the coefficient of degree I.  Each coefficient is an element of
!    the field of order P; in other words, each coefficient is
!    between 0 and P-1.
...
!    Input, integer ( kind = 4 ) P, the order of the field.
!
!    Output, integer ( kind = 4 ) PTOI, the (nonnegative) integer containing the 
!    polynomial information.
!
  implicit none
