...
!
  ...
  character ( len = 255 ) basis_file_name
  ...
  character ( len = 255 ) uv_file_name
  integer ( kind = 4 ) uv_file_num
  character ( len = 255 ) uv0_file_name(uv0_file_max)
...
!
!    Input, integer ( kind = 4 ) NODE(NPE,ELEMENT_NUM), the nodes that make up
!    each element.
!
...
!
  ...
  if ( comment ) then
    ...
    write ( file_out_unit, '(a)'       ) &
      '#  part of program POD_BASIS_FLOW.F90.'
    write ( file_out_unit, '(a)'       ) '#'
...
!*****************************************************************************80
!
!! DPBL_CHECK checks dimensions of a positive definite symmetric band matrix.
!
!  Discussion:
!
...
!    Input, character ( len = * ) STRING, the prompt string.
!
!    Output, integer ( kind = 4 ) VALUE1, VALUE2, values entered by the user.
!
!    Output, integer ( kind = 4 ) IERROR, an error flag, which is zero if 
!    no error occurred.
!
...
!*****************************************************************************80
!
!! TRIANGLE_UNIT_SET: weights and abscissas for quadrature in a unit triangle.
!
!  Integration region:
!
