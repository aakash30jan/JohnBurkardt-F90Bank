...
!    by using it to integrate weighted monomials over [0,+oo), and comparing the
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS_GEN_LAGUERRE'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the polynomial exactness ' // &
    'of a generalized Gauss-Laguerre'
  write ( *, '(a)' ) '  quadrature rule by integrating exponentially weighted '
  write ( *, '(a)' ) &
    '  monomials up to a given degree over the [0,+oo) interval.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  The rule may be defined on another interval, [A,+oo)'
  write ( *, '(a)' ) '  in which case it is adjusted to the [0,+oo) interval.'
!
...
!
  if ( 3 <= arg_num ) then
    ...
    if ( alpha <= - 1.0D+00 ) then
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'INT_EXACTNESS_GEN_LAGUERRE - Fatal error!'
      write ( *, '(a)' ) '  ALPHA <= -1.0.'
      stop
    end if
...
!  0 for the standard generalized rule for integrating x^alpha*exp(-x)*f(x),
...
!
  if ( 4 <= arg_num ) then
    ...
    write ( *, '(a)' ) &
      '  0: standard generalized rule for integrating x^alpha*exp(-x)*f(x);'
    write ( *, '(a)' ) &
      '  1: modified generalized rule for integrating                 f(x).'
    write ( *, '(a)' ) ' '
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS_GEN_LAGUERRE: User input:'
...
!
  ...
  write ( *, '(a)'       ) '  a generalized Gauss-Laguerre rule'
  ...
  if ( option == 0 ) then
    ...
    write ( *, '(a)'       ) '    Integral ( A <= x < +oo ) f(x) dx'
    write ( *, '(a)'       ) '    is to be approximated by'
    write ( *, '(a)'       ) '    sum ( 1 <= I <= ORDER ) w(i) * f(x(i)).'
  end if
  write ( *, '(a)'       ) ' '
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '          Error          Degree'
  write ( *, '(a)' ) ' '

  do degree = 0, degree_max

    call monomial_quadrature_gen_laguerre ( degree, alpha, order, &
    option, w, x, quad_error )
...
!*****************************************************************************80
!
!! GEN_LAGUERRE_INTEGRAL evaluates a generalized Laguerre monomial integral.
!
!  Discussion:
!
...
subroutine monomial_quadrature_gen_laguerre ( expon, alpha, order, option, &
  w, x, quad_error )
...
!    Input, integer ( kind = 4 ) EXPON, the exponent.
!
!    Input, real ( kind = 8 ) ALPHA, the exponent of X in the weight factor.
!
...
!
  call gen_laguerre_integral ( expon, alpha, exact )
!
...
!
  if ( option == 0 ) then
    ...
    value(1:order) = x(1:order)**alpha * exp ( - x(1:order) ) &
      * x(1:order)**expon
  end if
!
