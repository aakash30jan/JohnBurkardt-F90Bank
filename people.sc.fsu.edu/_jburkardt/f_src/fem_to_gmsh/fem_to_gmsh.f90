...
!
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  Create "prefix".msh, a corresponding Gmsh mesh file.'
!
...
!
  ...
  if ( m == 1 ) then
    ...
    if ( element_order == 2 ) then
      ...
      write ( *, * ) ' '
      write ( *, '(a)' ) 'FEM_TO_GMSH - Fatal error!'
      write ( *, '(a)' ) '  1D data must use 2 nodes.'
      stop 1
    end if
    ...
    if ( element_order == 3 ) then
      ...
      write ( *, * ) ' '
      write ( *, '(a)' ) 'FEM_TO_GMSH - Fatal error!'
      write ( *, '(a)' ) '  2D mesh data must use 3 or 6 nodes.'
      stop 1
    end if
    ...
    if ( element_order == 4 ) then
      ...
      write ( *, * ) ' '
      write ( *, '(a)' ) 'FEM_TO_GMSH - Fatal error!'
      write ( *, '(a)' ) '  3D mesh data must use 4, 10 or 20 nodes.'
      stop 1
    end if
...
!
  if ( m == 1 ) then
    call gmsh_mesh1d_write ( gmsh_filename, m, node_num, node_x, &
      element_order, element_num, element_node )
  else if ( m == 2 ) then
    call gmsh_mesh2d_write ( gmsh_filename, m, node_num, node_x, &
      element_order, element_num, element_node )
  else if ( m == 3 ) then
    call gmsh_mesh3d_write ( gmsh_filename, m, node_num, node_x, &
      element_order, element_num, element_node )
  end if

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Created the GMSH file "' &
    // trim ( gmsh_filename ) // '".'
!
!  Free memory.
!
