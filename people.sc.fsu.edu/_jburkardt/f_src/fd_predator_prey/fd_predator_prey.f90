...
!    a natural death rate; instead, they only die by being eaten by the 
...
!    eating prey, and the chances of this depend on the number of 
!    available prey.
!
...
!
!    Here, the initial values (5000,100) are a somewhat arbitrary 
!    starting point.
!
...
!    a periodic oscillation.  The two populations will be out of phase; the 
!    number of prey will rise, then after a delay, the predators will rise as 
!    the prey begins to fall, causing the predator population to crash again.
!
!    In this program, the pair of ODE's is solved with a simple finite 
!    difference approximation using a fixed step size.  In general, this is NOT 
!    an efficient or reliable way of solving differential equations.  However, 
!    this program is intended to illustrate the ideas of finite difference 
!    approximation.
!
...
!
  ...
  write ( *, '(a)' ) '  period and amplitude.  The finite difference'
  write ( *, '(a)' ) '  approximation can provide a good estimate for this'
  write ( *, '(a)' ) '  behavior if the stepsize DT is small enough.'
!
