...
!
  ...
  write ( s1, '(i1) ' ) m
  s1 = adjustl ( s1 )
  write ( xml_unit, '(a)' ) '  <mesh celltype="interval" dim="' // trim ( s1 ) // '">'
  ...
  do element = 1, element_num
    ...
    s3 = adjustl ( s3 )
    write ( xml_unit, '(a)' ) '      <interval index ="' &
      // trim ( s1 ) // &
      '" v0 ="' // trim ( s2 ) // &
      '" v1 ="' // trim ( s3 ) // '"/>'
...
!
  ...
  write ( s1, '(i1) ' ) m
  s1 = adjustl ( s1 )
  write ( xml_unit, '(a)' ) '  <mesh celltype="tetrahedron" dim="' // trim ( s1 ) // '">'
