...
!
  ...
  write ( *, '(a)' ) '           = 0 elsewhere.'
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  We modify the FTCS approach using the Lax method:'
  write ( *, '(a)' ) '    du/dt = (u(t+dt,x)-0.5*u(t,x-dx)-0.5*u(t,x+dx))/dt'
  write ( *, '(a)' ) '    du/dx = (u(t,x+dx)-u(t,x-dx))/2/dx'
...
!  curve at successive discrete times.  Instead, GNUPLOT wants to 
!  make a surface.
!
...
!
  ...
  do i = 1, nt
    ...
    do j = 1, nx
      ...
      unew(j) = 0.5D+00 * u(jm1) + 0.5D+00 * u(jp1) &
        - c * dt / dx / 2.0D+00 * ( u(jp1) - u(jm1) )
    end do
...
!
  ...
  write ( command_unit, '(a)' ) 'set output "advection_lax.png"'
