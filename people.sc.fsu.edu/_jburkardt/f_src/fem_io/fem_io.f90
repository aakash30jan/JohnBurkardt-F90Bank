...
!    coordinate file.  If this argument is not supplied, the program terminates.
!
...
!
  ...
  if ( 0 < len_trim ( node_data_file_name ) ) then
    ...
    if ( node_num2 /= node_num ) then
      write ( *, '(a)' ) ' '
      write ( *, '(a,i6)' ) &
        '  The number of nodes in the node coordinate file is ', node_num
      write ( *, '(a,i6)' ) &
        '  The number of nodes in the node data file is ', node_num2
...
!
  if ( 0 < len_trim ( node_coord_file_name ) ) then
    ...
    call r8mat_write ( node_coord_file_name, dim_num, node_num, node_coord )
...
!
  if ( 0 < len_trim ( element_file_name ) ) then
    ...
    call i4mat_write ( element_file_name, element_order, element_num, &
      element_node )
...
!
  if ( 0 < len_trim ( node_data_file_name ) ) then
    ...
    call r8mat_write ( node_data_file_name, node_data_num, node_num, node_data )
...
!    Input, character ( len = * ) FILE_NAME, the name of the file.
!
!    Output, integer ( kind = 4 ) LINE_NUM, the number of lines found 
!    in the file.
