...
!    * prefix.ps, the plot of the triangulation (output).
!
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TRIANGULATION_PLOT'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Read a node dataset of NODE_NUM points in 2 dimensions.'
  write ( *, '(a)' ) '  Read an associated triangulation dataset of '
  write ( *, '(a)' ) '  ELEMENT_NUM element in a triangulation of order'
  write ( *, '(a)' ) '  ELEMENT_ORDER = 3, 4 or 6 .'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Make an EPS plot of the triangulated data.'
!
...
!
!  Second argument is the node visibility.
!
  if ( 2 <= arg_num ) then

    iarg = 2
    call getarg ( iarg, string )
    call s_to_i4 ( string, node_show, ierror, length )

  else

    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '  Enter the option for showing the nodes:'
    write ( *, '(a)' ) '  0: do not show the nodes;'
    write ( *, '(a)' ) '  1:        show the nodes;'
    write ( *, '(a)' ) '  2:        show the nodes, and label them.'

    read ( *, * ) node_show

  end if
!
!  Third argument is the element visibility.
!
  if ( 3 <= arg_num ) then

    iarg = 3
    call getarg ( iarg, string )
    call s_to_i4 ( string, element_show, ierror, length )

  else

    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '  Enter the option for showing the elements:'
    write ( *, '(a)' ) '  0: do not show the elements;'
    write ( *, '(a)' ) '  1:        show the elements;'
    write ( *, '(a)' ) '  2:        show the elements, and label them.'
...
!
  if ( element_order == 3 ) then
    ...
    call triangulation_order3_plot ( plot_filename, node_num, node_xy, &
      element_num, element_node, node_show, element_show )

  else if ( element_order == 4 ) then

    call triangulation_order4_plot ( plot_filename, node_num, node_xy, &
      element_num, element_node, node_show, element_show )

  else if ( element_order == 6 ) then

    call triangulation_order6_plot ( plot_filename, node_num, node_xy, &
      element_num, element_node, node_show, element_show )

  end if

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Created the EPS file "' // trim ( plot_filename ) //'".'
!
!  Free memory.
!
  deallocate ( node_xy )
...
!  Discussion:
!
!    An I4MAT is a rectangular array of integer ( kind = 4 ) values.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(3,ELEMENT_NUM),  the indices of 
!    the points that form the vertices of each element.
!
...
!
  if ( 1 <= element_show ) then
    ...
    write ( file_unit, '(a)' ) '%'
    write ( file_unit, '(a)' ) '%  Draw the elements.'
    write ( file_unit, '(a)' ) '%'
...
!
  if ( 2 <= element_show ) then
    ...
    write ( file_unit, '(a)' ) '%'
    write ( file_unit, '(a)' ) '%  Label the elements:'
    write ( file_unit, '(a)' ) '%'
...
!    and the centroid of the element.
!
!    Input, integer ( kind = 4 ) NODE_SHOW,
!    0, do not show nodes;
!    1, show nodes;
...
!    Input, integer ( kind = 4 ) ELEMENT_NUM, the number of elements.
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(6,ELEMENT_NUM), lists, 
!    for each element, the indices of the points that form the vertices 
!    of the element.
!
!    Input, integer ( kind = 4 ) NODE_SHOW,
...
!  Local parameters:
!
!    Integer CIRCLE_SIZE, controls the size of the circles depicting
