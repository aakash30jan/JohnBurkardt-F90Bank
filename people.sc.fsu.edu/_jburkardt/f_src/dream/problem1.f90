...
!
  ...
  call r8mat_pofac ( par_num, c, c_factor )

  call r8mat_podet ( par_num, c_factor, c_det )
...
!
  ...
  call r8mat_utsol ( par_num, c_factor, x, y )
!
!  Compute:
!    (x-mu)' * inv(C)          * (x-mu)
!  = (x-mu)' * inv(R'*R)       * (x-mu)
...
!
  ...
  sample_likelihood = &
    - 0.5D+00 * real ( par_num, kind = 8 ) * log ( 2.0D+00 * pi ) &
    - 0.5D+00 * log ( c_det ) &
    - 0.5D+00 * xcx
