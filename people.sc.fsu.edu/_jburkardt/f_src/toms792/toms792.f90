...
!
!    Input, integer ( kind = 4 ) FLAG, indicates whether derivatives 
!    are desired.
...
!
  if ( k == 1 ) then
    ...
    f = 0.75 * exp ( - ( ( 9.0 * x - 2.0 )**2 &
      + ( 9.0 * y - 2.0 )**2 ) / 4.0 ) + &
      0.75 * exp ( - ( ( 9.0 * x + 1.0 )**2 ) &
      / 49.0 - ( 9.0 * y + 1.0 ) / 10.0 ) + &
      0.5 * exp ( - ( ( 9.0 * x - 7.0 )**2 &
      + ( 9.0 * y - 3.0 )**2 ) / 4.0 ) - &
    ...
    if ( 1 <= flag ) then
      ...
      fx = - 3.375 * ( 9.0 * x - 2.0 ) * t1 - ( 27.0 / 98.0 ) &
        * ( 9.0 * x + 1.0 ) * t2 &
...
!    The last four functions represent more challenging surface 
!    fitting problems.
!
!    Note that function 6 is only defined inside a circle of
!    radius 8/9 centered at (.5,.5).  Thus, if (X-.5)**2 +
...
!
  if ( k == 1 ) then
    ...
    if ( 1 <= flag ) then
      ...
      fx = - 3.375 * ( 9.0 * x - 2.0 ) * t1 &
        - ( 27.0 / 98.0 ) * ( 9.0 * x + 1.0 ) * t2 &
        - 2.25 * ( 9.0 * x - 7.0 ) * t3 + 3.6 * ( 9.0 * x - 4.0 ) * t4
