...
!  Discussion:
!
!    FEM_TO_MEDIT converts mesh data from FEM to MEDIT format.
!
...
!
  call fem_to_medit_handle ( node_filename, element_filename, &
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Writing MEDIT mesh file: "' // trim ( mesh_filename ) // '".'
...
subroutine medit_write ( filename, dim, vertices, edges, triangles, &
  quadrilaterals, tetrahedrons, hexahedrons, vertex_coordinate, &
  vertex_label, edge_vertex, edge_label, triangle_vertex, triangle_label, &
  quadrilateral_vertex, quadrilateral_label, tetrahedron_vertex, &
  tetrahedron_label, hexahedron_vertex, hexahedron_label )
...
!*****************************************************************************80
!
!! MEDIT_WRITE writes sizes and data to a MEDIT mesh file.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
  ...
  if ( ios /= 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'MEDIT_WRITE - Fatal error!'
    write ( *, '(a)' ) '  Could not open file.'
    stop
  end if

  write ( fileunit, '(a)' ) 'MeshVersionFormatted 1'
  write ( fileunit, '(a)' ) '#  Created by medit_write.f90'
!
