...
!
  ...
  integer ( kind = 4 ) seed
  character ( len = 255 ) steady_file_name
  ...
  real ( kind = 8 ) x_max
  real ( kind = 8 ) x_min
  character ( len = 255 ) xy_file_name
...
!
!    Input, integer ( kind = 4 ) ENERGY_IT_MAX, the maximum number of energy
!    iterations.
!
...
!
!    Input, integer ( kind = 4 ) NODE(NPE,ELEMENT_NUM), the nodes that make 
!    up each element.
!
...
!    the points.
!
!    Output, integer ( kind = 4 ) CLUSTER(POINT_NUM), the clusters to which 
!    points are assigned.
!
...
!
!  Extract integer ( kind = 4 ) information from the string.
!
...
!
!    The pair of I4's may be separated by spaces or a comma or both.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!    Input, character ( len = * ) STRING, the prompt string.
!
!    Output, integer ( kind = 4 ) VALUE1, VALUE2, the values entered
!    by the user.
!
!    Output, integer ( kind = 4 ) IERROR, an error flag, which is zero 
!    if no error occurred.
...
!*****************************************************************************80
!
!! ITABLE_DATA_READ reads data from an I4 table file.
!
!  Discussion:
!
...
!*****************************************************************************80
!
!! ITABLE_HEADER_READ reads the header from an I4 table file.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!    Input, real ( kind = 8 ) POINT(DIM_NUM,POINT_NUM), the data points.
!
!    Input/output, integer ( kind = 4 ) CLUSTER(POINT_NUM), the cluster to 
!    which each point belongs.  On output, these may have been altered.
!
!    Input/output, real ( kind = 8 ) CLUSTER_CENTER(DIM_NUM,CLUSTER_NUM),
!    the centers associated with the clustering.  On output, these may
!    have been altered.
...
!
!    Output, integer ( kind = 4 ) NEAREST, the index of the center point 
...
!    to be checked.
!
!    Input/output, integer ( kind = 4 ) CLUSTER(POINT_NUM).  On input, the 
!    cluster to which each point was assigned.  On output, the cluster to which
!    each point has been reassigned.
!
!    Output, integer ( kind = 4 ) SWAP_NUM, the number of times a point was 
...
!    entered by the user.
!
!    Output, integer ( kind = 4 ) IERROR, an error flag, which is zero if no
!    error occurred.
!
  implicit none
...
!
!    Output, integer ( kind = 4 ) IERROR, an error flag, which is zero if 
!    no error occurred.
!
  implicit none
...
!    value.  The representation will be left-justified.
!
  implicit none
...
!
!  The absolute value goes into S(ILO:IHI).
!
  ipos = ihi
!
