...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TRIANGLE_NCC_RULE_PRB:'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Test the TRIANGLE_NCC_RULE library.'
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST01'
  write ( *, '(a)' ) '  TRIANGLE_NCC_RULE_NUM returns the number of rules;'
  write ( *, '(a)' ) '  TRIANGLE_NCC_DEGREE returns the degree of a rule;'
  write ( *, '(a)' ) '  TRIANGLE_NCC_ORDER_NUM returns the order of a rule.'

  call triangle_ncc_rule_num ( rule_num )
  ...
  do rule = 1, rule_num
    call triangle_ncc_order_num ( rule, order_num )
    call triangle_ncc_degree ( rule, degree )
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST02'
  write ( *, '(a)' ) '  TRIANGLE_NCC_RULE returns the points and weights'
  write ( *, '(a)' ) '  of an NCC rule for the triangle.'
  write ( *, '(a)' ) ' '
  ...
  do rule = 1, rule_num
    ...
    call triangle_ncc_rule ( rule, order_num, xytab, wtab )
...
!
  ...
  do rule = 1, rule_num
    ...
    call triangle_ncc_suborder_num ( rule, suborder_num )

    allocate ( suborder_w(1:suborder_num) )
    allocate ( suborder_xyz(1:3,1:suborder_num) )

    call triangle_ncc_subrule ( rule, suborder_num, suborder_xyz, suborder_w )
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST04'
  write ( *, '(a)' ) '  TRIANGLE_NCC_RULE returns the points and weights of'
  write ( *, '(a)' ) '  an NCC rule for the unit triangle.'
  write ( *, '(a)' ) ' '
...
!
  ...
  real ( kind = 8 ), dimension ( dim_num, node_num ) :: node_xy2 = &
    reshape ( (/ &
    1.0D+00, 2.0D+00, &
    1.0D+00, 1.0D+00, &
  ...
  call triangle_ncc_rule ( rule, order_num, xy, w )
!
