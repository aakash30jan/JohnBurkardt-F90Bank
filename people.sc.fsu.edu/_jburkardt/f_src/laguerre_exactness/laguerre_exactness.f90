...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the exactness of a Gauss-Laguerre'
...
!
  if ( 3 <= arg_num ) then
    ...
    write ( *, '(a)' ) '  0: standard rule for integrating exp(-x)*f(x);'
    write ( *, '(a)' ) '  1: modified rule for integrating         f(x).'
    write ( *, '(a)' ) ' '
...
!
  ...
  if ( option == 0 ) then
    write ( *, '(a,g14.6,a)' ) '  Integral ( ', a, ' <= x < +oo ) f(x) exp ( -x ) dx'
  else if ( option == 1 ) then
    write ( *, '(a,g14.6,a)' ) '  Integral ( ', a, ' <= x < +oo ) f(x) dx'
  end if
  write ( *, '(a)'       ) ' '
  write ( *, '(a)'       ) '  Weights W:'
  write ( *, '(a)'       ) ' '
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  A Gauss-Laguerre rule would be able to exactly'
  write ( *, '(a,i8)' ) &
  ...
  do degree = 0, degree_max
    ...
    call laguerre_monomial_quadrature ( degree, order, option, w, x, &
      quad_error )
...
subroutine laguerre_monomial_quadrature ( expon, order, option, w, x, &
  quad_error )
