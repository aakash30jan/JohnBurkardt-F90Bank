...
!   * prefix_l2q_nodes.txt contains the new node coordinates;
!   * prefix_l2q_elements.txt contains the new element definitions.
!
!  Licensing:
!
...
!
  ...
  integer ( kind = 4 ) element_order1
  integer ( kind = 4 ), parameter :: element_order2 = 10
  integer ( kind = 4 ) iarg
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TET_MESH_L2Q'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Read a "linear" tet mesh and'
  write ( *, '(a)' ) '  write out a "quadratic" tet mesh.'

  if ( verbose ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '  Read a dataset of NODE_NUM1 points in 3 dimensions.'
    write ( *, '(a)' ) '  Read an associated tet mesh of TETRA_NUM'
    write ( *, '(a)' ) '  tetrahedrons, using 4 nodes per tetrahedron.'
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '  Create new nodes which are midpoints of sides,'
    write ( *, '(a)' ) '  generate new node and tet mesh data for'
    write ( *, '(a)' ) '  quadratic 10-node tetrahedrons, and write them out.'
  end if
!
...
!
  ...
  output_node_filename = trim ( prefix ) // '_l2q_nodes.txt'
  output_element_filename = trim ( prefix ) // '_l2q_elements.txt'
!
!  Read the node data.
!
  ...
  if ( dim_num /= 3 ) then
    write ( *, * ) ' '
    write ( *, '(a)' ) 'TET_MESH_L2Q - Fatal error!'
...
!
  call i4mat_header_read ( input_element_filename, element_order1, element_num )

  if ( element_order1 /= 4 ) then
    write ( *, * ) ' '
    write ( *, '(a)' ) 'TET_MESH_L2Q - Fatal error!'
    write ( *, '(a)' ) '  Data is not for 4-node tetras.'
    stop 1
  end if
  ...
  if ( verbose ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a,i8)' ) '  Tetrahedron order = ', element_order1
  end if

  write ( *, '(a,i8)' ) '  Number of elements = ', element_num

  allocate ( element_node1(1:element_order1,1:element_num) )

  call i4mat_data_read ( input_element_filename, element_order1, element_num, &
    element_node1 )
  ...
  if ( verbose ) then
    ...
    call i4mat_transpose_print_some ( element_order1, element_num, element_node1, &
      1, 1, element_order1, 5, '  First 5 elements:' )
  end if
!
!  Detect and correct 0-based indexing.
!
  call mesh_base_one ( node_num1, element_order1, element_num, element_node1 )
!
...
!
  ...
  allocate ( element_node2(1:element_order2,1:element_num) )
...
!
  ...
  call i4mat_transpose_print_some ( element_order2, element_num, element_node2, &
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Wrote the quadratic nodes to "' &
    // trim ( output_node_filename ) //'".'

  call i4mat_write ( output_element_filename, element_order2, element_num, &
    element_node2 )

  write ( *, '(a)' ) '  Wrote the quadratic elements to "' &
    // trim ( output_element_filename ) //'".'
!
!  Free memory.
!
  deallocate ( edge_data )
  deallocate ( element_node1 )
