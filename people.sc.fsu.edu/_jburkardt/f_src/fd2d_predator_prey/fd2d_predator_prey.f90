...
!
!    Local parameter, integer ( kind = 4 ) BIGJ, the number of intervals in 
!    either spatial coordinate.
!
!    User input, real ( kind = 8 ) DELT, the time step to use.
!
!    User input, real ( kind = 8 ) DELTA, the value of the parameter DELTA.
!
!    Local parameter, integer ( kind = 4 ) DIMJ, the number of nodes in either
!
...
!
!    Local parameter, integer ( kind = 4 ) N, the total number of spatial nodes.
!
!    Local parameter, integer ( kind = 4 ) SOLVE, 0 for GMRES routine, 
!    1 for Jacobi.
!
...
!
  ...
  integer ( kind = 4 ), parameter :: maxl = 10
  ...
  real ( kind = 8 ) alpha
  real ( kind = 8 ) b
  integer ( kind = 4 ), allocatable, dimension ( : ) :: b_i
  integer ( kind = 4 ), allocatable, dimension ( : ) :: b_j
  integer ( kind = 4 ), allocatable, dimension ( : ) :: b_ppm
  ...
  integer ( kind = 4 ), allocatable, dimension ( : ) :: g_ppm
  real ( kind = 8 ) gamma
  real ( kind = 8 ) h
  real ( kind = 8 ), allocatable, dimension ( : ) :: hhat
  integer ( kind = 4 ) i
  integer ( kind = 4 ) i2
  integer ( kind = 4 ) idummy(1)
  integer ( kind = 4 ) ierr
  ...
  integer ( kind = 4 ), allocatable, dimension ( : ) :: r_ppm
  real ( kind = 8 ), allocatable, dimension ( : ) :: rgwk
  integer ( kind = 4 ) solve
  real ( kind = 8 ) t
...
!
!    Input/output, integer ( kind = 4 ) A1(N), integer A2(N), the data 
!    to be sorted.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) I, J, the row and column index of the entry.
!
...
!
!    Input/output, integer ( kind = 4 ) NZ_NUM, is used in MODE = 0 and 1 only.
...
!
!    Input/output, integer ( kind = 4 ) NZ, is used only in MODE = 0 and 2.
...
!
!    Input/output, integer ( kind = 4 ) B_I(*), B_J(*), 
!    real ( kind = 8 ) B_V(*).  These vectors store the sparse matrix data.  
!    Presumably, a dimension of NZ_NUM (determined with MODE = 1) would be 
!    enough to store the entries (assigned with MODE = 2).  Initially, the 
!    data is stored as the user reports it, which means that the data is
!    unsorted, and that the same row and column indices may appear multiple
!    times.
!
  implicit none
