...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TABLE_LATINIZE'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Read a dataset of N points in M dimensions,'
  write ( *, '(a)' ) '  modify it into a Latin hypercube,'
...
!
  ...
  do i = 1, dim_num
    ...
    do j = 1, n
      table(i,indx(j)) = real ( 2 * j - 1, kind = 8 ) &
                       / real ( 2 * n    , kind = 8 )
    end do
  end do
...
subroutine s_to_r8vec ( s, n, dvec, ierror )
...
!
!    Output, real ( kind = 8 ) DVEC(N), the values read from the string.
!
!    Output, integer ( kind = 4 ) IERROR, error flag.
!    0, no errors occurred.
...
!
  ...
  do while ( i < n )
    ...
    call s_to_r8 ( s(ilo:), dvec(i), ierror, lchar )
