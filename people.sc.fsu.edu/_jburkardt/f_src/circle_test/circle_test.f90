...
!
  ...
  r8_pi = 3.14159265358979323846264338327950288419716939937510D+00
...
!
!    Output, real ( kind = 8 ) RADIUS(POINT_NUM), the radius of the sphere
!    around each point.
!
  implicit none
  ...
  if ( walls ) then
         if ( any (           coord(1:dim_num,1:point_num) < 0.0D+00 ) ) then
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'RADIUS_MAXIMUS - Fatal error!'
      write ( *, '(a)' ) '  Some coordinate is less than 0.'
      stop
    else if ( any ( 1.0D+00 < coord(1:dim_num,1:point_num)           ) ) then
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'RADIUS_MAXIMUS - Fatal error!'
