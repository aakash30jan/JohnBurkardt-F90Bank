...
!
!    Output, integer ( kind = 4 ) OBJ_TEXT_NUM, the number of lines in the 
!    OBJ file.
!
  implicit none
...
!  Parameters:
!
!    Input, character ( len = * ) OBJ_FILE_UNIT, the unit number of the 
!    output file.
!
...
!
!    Input/output, integer ( kind = 4 ) OBJ_TEXT_NUM, the number of lines of 
!    text in the OBJ file.
!
  implicit none
  ...
  do face = 1, face_num
    ...
    if ( normal_num <= 0 ) then
      ...
      do vertex = 1, face_order(face)
        text2 = ' '
        write ( text2(2:), '(i8, ''//'', i8 )' ) &
          face_node(vertex,face) + node_offset, &
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) TEC_FILE_UNIT, the unit associated with 
!    the file.
!
...
!
!    Input, integer ( kind = 4 ) ELEMENT_ORDER_UNIFORM, the order of 
!    the elements.
!
!    Input, integer ( kind = 4 ) NODE_DATA_NUM, the number of data items 
!    per node.
!
...
!    ELEMENT_NODE(ELEMENT_ORDER_UNIFORM,ELEMENT_NUM); 
...
!  READ_STATUS = 1: We've read "ZONE", haven't seen the end of the zone stuff.
...
!  "Name=Value" items,
!
!  We are interested in node, element and element order information.
!
...
!
      do
        ...
        elseif ( &
          ( ch_eqi ( name, 'E' ) .or. ch_eqi ( name, 'ELEMENTS' ) ) .and. &
          element_num == -1 ) then
...
!
  ...
  integer ( kind = 4 ) element_order_uniform
  integer ( kind = 4 ) element_total
  character ( len = 255 ) element_type
  character ( len = 255 ) line
...
!
  ...
  call obj_header_write ( obj_file_name, obj_file_unit, tec_file_name, &
    obj_text_num )
!
...
!
  ...
  do zone = 1, zone_num
    ...
    call tec_zone_data_read ( tec_file_unit, line, dim_num, node_num, &
      element_num, element_order_uniform, node_data_num, node_coord, &
      element_node, node_data )
