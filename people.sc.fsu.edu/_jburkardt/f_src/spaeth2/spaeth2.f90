...
!
!    Input, integer ( kind = 4 ) METHOD, specifies the method to be used.  
!    METHOD should be between 1 and 7.
!
!    Output, integer ( kind = 4 ) A(M-1), B(M-1), contains the two indices of
!    the two clusters being merged at each step.
!
...
!    Input, integer ( kind = 4 ) NC, the number of clusters.
!
!    Output, integer ( kind = 4 ) P(M), the cluster to which each item 
!    is assigned.
!
...
!
!    Input, integer ( kind = 4 ) ITMAX, the maximum number of 
!    iterations allowed.
!
