...
!    residual vectors that are to be downdated.  On output, the downdated norms.
!
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) R(LDR,P); On input, the upper triangular
!    factor that is to be updated.  On output, the updated factor.  Elements
!    below the diagonal are not referenced.
!
!    Input, integer ( kind = 4 ) LDR, the leading dimension of R, which is at
!    least P.
!
!    Input, integer ( kind = 4 ) P, the order of the matrix.
!
...
!
!    Input/output, real ( kind = 4 ) RHO(NZ); on input, the norms of the 
!    residual vectors that are to be updated.  If RHO(J) is negative, it is
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) ABD(LDA,N), on input, contains the 
...
!
!    Input, integer ( kind = 4 ) ML, the number of diagonals below the 
!    main diagonal.  0 <= ML < N.
!
!    Input, integer ( kind = 4 ) MU, the number of diagonals above the 
!    main diagonal.  0 <= MU < N.
...
!
!    Input, integer ( kind = 4 ) MU, the number of diagonals above the 
!    main diagonal.  0 <= MU < N.  More efficient if ML <= MU.
!
!    Output, integer ( kind = 4 ) IPVT(N), the pivot indices.
!
!    Output, integer ( kind = 4 ) INFO.
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 )A(LDA,N), on input, the matrix to be 
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) A(LDA,N); on input, the factor
!    information from CGECO or CGEFA.  On output, the inverse matrix, if it
!    was requested,
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of A.
!
...
!
!    Output, complex ( kind = 4 ) DET(2), the determinant of the original
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) A(LDA,N); on input, the hermitian matrix
...
!    of a hermitian matrix A.  The columns of the upper triangle are stored
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) ABD(LDA,N); on input, the matrix to be
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, integer ( kind = 4 ) M, the number of diagonals above the
!    main diagonal.  0 <= M < N.
!
!    Output, real ( kind = 4 ) RCOND, an estimate of the reciprocal condition of
!    the matrix.  For the system A*X = B, relative perturbations in A and B
...
!
!    Output, real ( kind = 4 ) DET(2), the determinant of the original matrix
!    in the form determinant = DET(1) * 10.0**DET(2) with 1.0 <= DET(1) < 10.0
!    or DET(1) == 0.0.
!
  implicit none
...
!
  ...
  do j = 1, n
    ...
    do k = mu, m
      t = abd(k,j) - cdotc ( k-mu, abd(ik:ik+k-mu-1,jk), 1, &
        abd(mu:mu+k-mu-1,j), 1 )
      t = t / abd(m+1,jk)
      abd(k,j) = t
      s = s + real ( t * conjg ( t ) )
      ik = ik - 1
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) A(LDA,N); on input, the hermitian 
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) A(LDA,N); on input, the output A from 
!    CPOCO or CPOFA, or the output X from CQRDC.  On output, if CPOCO or CPOFA 
!    was used to factor A, then CPODI produces the upper half of inverse(A).
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) AP(N*(N+1)/2); on input, the packed
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) A((N*(N+1))/2); on input, the output
!    from CPPCO or CPPFA.  On output, the upper triangular half of the inverse.
!    The strict lower triangle is unaltered.
!
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Output, real ( kind = 4 ) DET(2), the determinant of original matrix if 
!    requested.  Otherwise not referenced.  Determinant = DET(1) * 10.0**DET(2)
...
!
!    Input/output, complex ( kind = 4 ) E(N).  On input, the superdiagonal 
...
!
!    Input, complex ( kind = 4 ) Y(N), a vector that is to be manipulated 
!    by CQRSL.
!
...
!    of the matrix Q.
!
!    Output, complex ( kind = 4 ) B(K), the solution of the least squares 
!    problem
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) A(LDA,N); on input, the output from 
!    CSIFA.  If the inverse was requested, then on output, A contains the upper
!    triangle of the inverse of the original matrix.  The strict lower triangle
!    is never referenced.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of A.
!
...
!
!    Output, complex ( kind = 4 ) DET(2), if requested, the determinant of the
!    matrix.  Determinant = DET(1) * 10.0**DET(2) with 
!    1.0 <= abs ( DET(1) ) < 10.0 or DET(1) = 0.0.  Also, DET(2) is
!    strictly real.
!
!    Workspace, complex WORK(N).
!
...
!
!    Input/output, complex ( kind = 4 ) AP(N*(N+1)/2); on input, the packed form
!    of a symmetric matrix.  The columns of the upper triangle are stored
...
!  Parameters:
!
!    Input/output, complex ( kind = 4 ) AP(N*(N+1)/2); on input, the matrix
!    factors from CSPFA.  On output, if the inverse was requested, the upper
...
!    Output, complex ( kind = 4 ) DET(2), the determinant of the original 
!    matrix.  Determinant = DET(1) * 10.0**DET(2) with 
!    1.0 <= abs ( DET(1) ) < 10.0 or DET(1) = 0.0.  Also, DET(2) 
!    is strictly real.
!
!    Workspace, complex WORK(N).
!
