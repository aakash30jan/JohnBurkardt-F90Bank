subroutine qwv_2d ( t, n, a, b, c, d, x, y, w )
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) T, the desired total degree.
!    0 <= T.
!
!    Input, integer ( kind = 4 ) N, the number of points in the rule.
!    It should be the case that T = (N+1)*(N+2)/2.
!
!    Input, real ( kind = 8 ) A, B, C, D the endpoints of the X and Y intervals.
!
...
!
  ...
  do j = 0, t
    do i = 0, t - j
      ...
      rhs(k) = ( b ** ( i + 1 ) - a ** ( i + 1 ) ) / real ( i + 1, kind = 8 ) &
             * ( d ** ( j + 1 ) - c ** ( j + 1 ) ) / real ( j + 1, kind = 8 )
    end do
  end do
