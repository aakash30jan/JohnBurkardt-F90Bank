...
!
!    Input, integer ( kind = 4 ) NB, the bandwidth of G, not counting 
!    the diagonal.
!
...
!    data is read in.
!
!    Input, integer ( kind = 4 ) MT, is set by the user to indicate the number 
!    of new rows of data being introduced by the current call.
!    MT must be at least 0.
!
!    Input, integer ( kind = 4 ) JT, set by the user to indicate the column of 
!    the submatrix A(I) that is identified with the first column of C(I).
...
!
!    Input, integer ( kind = 4 ) LPIVOT, is the index of the pivot element, 
!    which should satisfy 1 <= LPIVOT < L1.
!
!    Input, integer ( kind = 4 ) L1, M.   If L1 <= M, the transformation will 
!    be constructed to zero out elements indexed from L1 through M.   
!    But if M < L1, the subroutine does an identity transformation.
!
!    Input/output, real ( kind = 8 ) U(IUE,M), integer ( kind = 4 ) IUE, 
!    real ( kind = 8 ) UP.
!    If MODE = 1:
!      On input, 
...
!
!    Input, integer ( kind = 4 ) ICE, the storage increment between successive 
!    elements of a single vector in C.
!
!    Input, integer ( kind = 4 ) ICV, the storage increment between the first 
!    element of succesive vectors in C.
!
!    Input, integer ( kind = 4 ) NCV, the number of vectors in C to be 
!    transformed.  If NCV <= 0, no operations are done on C.
!
  implicit none
...
!
!    Input, integer ( kind = 4 ) UNIT, the output unit.  If 0 <= UNIT, then 
!    output is to Fortran unit UNIT.  If UNIT == -1, output is to unit "*".
!
!    Input, integer ( kind = 4 ) WIDTH, the width of the output lines.  Each 
!    output line from this subroutine will have at most
...
!
!    Input, integer ( kind = 4 ) MDATA, the number of rows in the actual least 
!    squares problem.  Generally M <= MDATA.  MDATA is used only in computing
