...
!    cell generators.
!
!    Input, integer ( kind = 4 ) NBIN(3) is the number of bins to use in 
!    each direction.  For 3D problems, set NBIN(3) = 1.
!    For efficiency, these values should be set in such a way that the bins
!    are nearly square or cubical.
!
!    Output, integer ( kind = 4 ) BIN_START(NBIN(1),NBIN(2),NBIN(3)), the index 
!    of the first cell generator in the bin, or -1 if none.
!
!    Output, integer ( kind = 4 ) BIN_LAST(NBIN(1),NBIN(2),NBIN(3)), the index 
!    of the last cell generator in the bin, or -1 if none.
!
!    Output, integer ( kind = 4 ) BIN_NEXT(N), the index of the next cell 
!    generator in the bin containing this cell generator.
!
  implicit none
  ...
  integer ( kind = 4 ), dimension ( ndim ) :: nbin

  integer ( kind = 4 ), dimension ( nbin(1),nbin(2),nbin(3) ) :: bin_last
  integer ( kind = 4 ), dimension ( n ) :: bin_next
  integer ( kind = 4 ), dimension ( nbin(1),nbin(2),nbin(3) ) :: bin_start 
  real ( kind = 8 ), dimension ( ndim ) :: box_min
  real ( kind = 8 ), dimension ( ndim ) :: box_max
  real ( kind = 8 ), dimension ( ndim, n ) :: cell_generator
  integer ( kind = 4 ) empty
  logical, save :: first_call = .true.
  integer ( kind = 4 ) i
  integer ( kind = 4 ) j
  integer ( kind = 4 ) k
  integer ( kind = 4 ) nonempty
...
!
!    Input, integer ( kind = 4 ) NS_CVT, the number of sample points 
!    per generator.
!
...
!    physical region.
!
!    Input/output, integer ( kind = 4 ) UPDATES(N), counts the number of times 
!    a cell center has been updated.  Before the first call, all the entries of
...
!
!    Input, integer ( kind = 4 ) NBIN(3) is the number of bins to use in each 
!    direction.  For 3D problems, set NBIN(3) = 1.
!    For efficiency, these values should be set in such a way that the bins
!    are nearly square or cubical.
!
!    Input, integer ( kind = 4 ) BIN_START(NBIN(1),NBIN(2),NBIN(3)), the index
!    of the first cell generator in the bin, or -1 if none.
!
!    Input, integer ( kind = 4 ) BIN_LAST(NBIN(1),NBIN(2),NBIN(3)), the index 
!    of the last cell generator in the bin, or -1 if none.
!
!    Input, integer ( kind = 4 ) BIN_NEXT(N), the index of the next cell
!    generator in the bin containing this cell generator.
!
  implicit none

  integer ( kind = 4 ) n
  integer ( kind = 4 ) nbin(3)
  integer ( kind = 4 ) ndim
...
!    physical region.
!
!    Input, integer ( kind = 4 ) RANDOM_GENERATOR, specifies how the
...
!
!    Output, integer ( kind = 4 ) I_MIN, the index of the nearest point in PSET
!    to PTEST.
!
  implicit none
...
!    in the set.
!
!    Input, integer ( kind = 4 ) NBIN(3), the number of cells in the X, Y and 
!    Z directions.
!
...
!
!    Output, integer ( kind = 4 ) BIN_NEXT(N), contains the index of the next 
...
!    in the bin.
!
!    Input/output, integer ( kind = 4 ) BIN_NEXT(N), contains the index of 
!    the next element of A that follows this element in the same bin.  A value 
!    of 0 means this is the last entry in the particular bin.
!
  implicit none
...
!    or -1 if there are no entries in the bin.
!
!    Input/output, integer ( kind = 4 ) BIN_NEXT(N), contains the index of the 
!    next element of A that follows this element in the same bin.  A value of 0
!    means this is the last entry in the particular bin.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) M, the row dimension of A, and the length 
!    of a column.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) M, the row order of A, and the length 
!    of a column.
!
...
!
!    Input, integer ( kind = 4 ) RANDOM_FUNCTION, specifies the random function.
...
!    FALSE than a much simplified routine is used.
!
!    Output, integer ( kind = 4 ) NGEN, the number of points that were 
!    generated.  This is at least 1, but may be larger if some points were
!    rejected.
!
  implicit none
  ...
  real ( kind = 8 ) box_min(ndim)
  real ( kind = 8 ) dr
  integer ( kind = 4 ), save, dimension ( 3 ) :: halton_base = (/ 2, 3, 5 /)
  integer ( kind = 4 ), save :: halton_seed = 1
  integer ( kind = 4 ) ival
  real ( kind = 8 ) mdens
  integer ( kind = 4 ) ngen
  real ( kind = 8 ) r(ndim)
  integer ( kind = 4 ) random_function
  logical reset
  logical use_diatom
  real ( kind = 8 ) x(ndim)
...
!    Input, real ( kind = 8 ) CELL_GENERATOR(NDIM,N), the cell generators.
!
!    Input, integer ( kind = 4 ) NS_MOM, the number of sample points per 
!    cell generator.
!
!    Input, logical USE_DIATOM, is TRUE if DIATOM is to be called 
!    to determine whether a point lies in the physical region; if it is
...
!
  ...
  integer ( kind = 4 ) nearest
  integer ( kind = 4 ) ngen
  integer ( kind = 4 ) ns_mom
  integer ( kind = 4 ) ntries
  integer ( kind = 4 ), parameter :: random_mom = 0
  real ( kind = 8 ) region_volume
