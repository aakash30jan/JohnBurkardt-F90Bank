...
!
  ...
  if ( test == 1 ) then
    a = reshape ( (/ &
      ...
      ( 0.0D+00, 0.0D+00 ), ( 2.0D+00, 0.0D+00 ) /), (/ 2, 2 /) )
  else if ( test == 2 ) then
    a = reshape ( (/ &
      ( 0.0D+00, 3.0D+00 ), (  0.0D+00,  0.0D+00 ), &
      ( 0.0D+00, 0.0D+00 ), (  0.0D+00, -4.0D+00 ) /), (/ 2, 2 /) )
  else if ( test == 3 ) then
    a = reshape ( (/ &
      ( 5.0D+00, 6.0D+00 ), ( 0.0D+00,  0.0D+00 ), &
      ( 0.0D+00, 0.0D+00 ), ( 7.0D+00, -8.0D+00 ) /), (/ 2, 2 /) )
  else
    write ( *, '(a)' ) ' '
...
!*****************************************************************************80
!
!! C8MAT_EXP_EXPA returns the "exact" exponential matrix for a complex test.
!
!  Discussion:
!
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Output, complex ( kind = 8 ) EXPA(N,N), the exponential of the test matrix.
!
  implicit none

  integer ( kind = 4 ) n

  complex ( kind = 8 ) expa(n,n)
  integer ( kind = 4 ) test
  ...
  if ( test == 1 ) then
    expa = reshape ( (/ &
      ...
      ( 0.0D+00,               0.0D+00 ), &
      ( 0.0D+00,               0.0D+00 ), &
      ( 7.389056098930650D+00, 0.0D+00 ) /), (/ 2, 2 /) )
  else if ( test == 2 ) then
    expa = reshape ( (/ & 
      ( -0.989992496600446D+00, 0.141120008059867D+00 ), &
      ( 0.0D+00,               0.0D+00 ), &
      ( 0.0D+00,               0.0D+00 ), &
      ( -0.653643620863612D+00, 0.756802495307928D+00 ) /), (/ 2, 2 /) )
  else if ( test == 3 ) then
    expa = reshape ( (/ &
      ( 142.501905518208D+00, - 41.468936789923D+00 ), &
      ( 0.0D+00,               0.0D+00 ), &
      ( 0.0D+00,               0.0D+00 ), &
      ( -159.560161626987D+00, - 1084.963058811836D+00 ) /), (/ 2, 2 /) )
  else
    write ( *, '(a)' ) ' '
...
!*****************************************************************************80
!
!! C8MAT_EXP_STORY prints explanatory text for each complex test.
!
!  Discussion:
!
!     1) Real diagonal example
...
!
  ...
  if ( test == 1 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '  This matrix is diagonal.'
    write ( *, '(a)' ) '  The diagonal entries are pure imaginary.'
  else if ( test == 3 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '  This matrix is diagonal.'
...
!*****************************************************************************80
!
!! R8MAT_EXP_A returns the matrix for a real test.
!
!  Discussion:
!
!     1) Diagonal example
...
!*****************************************************************************80
!
!! R8MAT_EXP_EXPA returns the "exact" exponential matrix for a real test.
!
!  Discussion:
!
!    In some cases, the "exact" value is actually a numerical value
!    given to a limited number of significant digits.
!
!     1) Diagonal example
!     2) Symmetric example
!     3) Laub
  ...
  if ( test == 1 ) then
    ...
    expa = reshape ( (/ &
      -0.7357587581447531D+00, -1.4715175990882605D+00, &
       0.5518190996580977D+00,  1.1036382407155727D+00 /), (/ 2, 2 /) )
  else if ( test == 5 ) then
    expa = reshape ( (/ &
      1.0D+00,  0.0D+00, 0.0D+00, 0.0D+00, &
    ...
    expa = reshape ( (/ &
      2.718281828459045235360287D+00, 0.0D+00, &
      2.718281828459045235360287D+00, 2.718281828459045235360287D+00 /), &
      (/ 2, 2 /) )
  else if ( test == 8 ) then
    exp16 = exp ( 16.0D+00 )
    exp4 = exp ( 4.0D+00 )
...
!*****************************************************************************80
!
!! R8MAT_EXP_N returns the matrix order for a real test.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!*****************************************************************************80
!
!! R8MAT_EXP_STORY prints explanatory text for each real test.
!
!  Discussion:
!
!     1) Diagonal example
