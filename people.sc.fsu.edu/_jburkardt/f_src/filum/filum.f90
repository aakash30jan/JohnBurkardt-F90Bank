...
!
!    Output, logical ( kind = 4 ) CH_IS_DIGIT, .TRUE. if the character is a digit, 
!    FALSE otherwise.
!
  implicit none
...
!
  ...
  integer ( kind = 4 ) ios
  integer ( kind = 4 ) iunit
  logical ( kind = 4 ), parameter :: verbose = .false.
!
!  Does the file exist?
!
...
!    Input, character ( len = * ) filename, the name of the file.
!
!    Output, logical ( kind = 4 ) FILE_EXIST, is TRUE if the file exists.
!
  implicit none
...
!    the file.
!
!    Input/output, logical ( kind = 4 ) MORE.  On first call, the user should 
!    set MORE to be FALSE, which signals the routine to initialize.  Each time 
!    the routine reads another integer from the file, it returns it in VALUE, 
...
!    Input, character ( len = * ) filename, the name of the file.
!
!    Output, logical ( kind = 4 ) FILE_IS_OPEN, is TRUE if the file is open.
!
  implicit none
...
!    parentheses marks.
!
!    Output, logical ( kind = 4 ) FILE_TAG_CHECK, is true if the file passed the check.
!
  implicit none

  character ( len = * ) filename
  logical ( kind = 4 ) file_tag_check
  integer ( kind = 4 ) file_unit
  integer ( kind = 4 ) ios
  character ( len = * ) left
...
!    Input, character ( len = * ) filename, the name of the file.
!
!    Output, integer ( kind = 4 ) WORD_NUM, the number of words in the file.
!
  implicit none
