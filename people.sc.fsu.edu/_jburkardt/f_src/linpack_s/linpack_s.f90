...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) A(LDA,P).
...
!
!    Input, real ( kind = 4 ) WORK(P) is a work array.
!
    ...
    do k = 1, p
      ...
      if ( swapk ) then
        ...
        if ( k /= pl ) then
          ...
          call sswap ( pl-1, a(1:pl-1,k), 1, a(1:pl-1,pl), 1 )
    ...
    do k = p, pl, -1
      ...
      if ( ipvt(k) < 0 ) then
        ...
        if ( pu /= k ) then
          ...
          call sswap ( k-1, a(1:k-1,k), 1, a(1:k-1,pu), 1 )
...
!
    if ( k /= maxl ) then
      call sswap ( k-1, a(1:k-1,k), 1, a(1:k-1,maxl), 1 )
      a(maxl,maxl) = a(k,k)
      a(k,k) = maxdia
      jp = ipvt(maxl)
      ipvt(maxl) = ipvt(k)
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) R(LDR,P), the upper triangular matrix that
...
!
!    Input, real ( kind = 4 ) X(P), the row vector that is to be removed from R.
!
!    Input/output, real ( kind = 4 ) Z(LDZ,NZ), an array of NZ P-vectors
...
!
!    Input, real ( kind = 4 ) Y(NZ), the scalars for the downdating of
!    the vectors Z.
!
!    Input/output, real ( kind = 4 ) RHO(NZ), the norms of the residual vectors.
!    On output these have been changed along with R and Z.
!
!    Output, real ( kind = 4 ) C(P), S(P), the cosines and sines of the
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) R(LDR,P).  On input, the upper
...
!
!    Input/output real Z(LDZ,NZ), an array of NZ P-vectors into
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) R(LDR,P), the upper triangular matrix
...
!    Input, integer ( kind = 4 ) P, the order of the matrix R.
!
!    Input, real ( kind = 4 ) X(P), the row to be added to R.
!
!    Input/output, real ( kind = 4 ) Z(LDZ,NZ), contains NZ P-vectors
!    to be updated with R.
!
!    Input, integer ( kind = 4 ) LDZ, the leading dimension of the array Z.
!    LDZ must be at least P.
!
!    Input, integer ( kind = 4 ) NZ, the number of vectors to be updated.
!    NZ may be zero, in which case Z, Y, and RHO are not referenced.
!
!    Input, real ( kind = 4 ) Y(NZ), the scalars for updating the vectors Z.
!
!    Input/output, real ( kind = 4 ) RHO(NZ).  On input, the norms of the
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) ABD(LDA,N).  On input, the matrix in band
...
!  Parameters:
!
!    Input, real ( kind = 4 ) ABD(LDA,N), the LU factor
!    information from SGBCO or SGBFA.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of the array ABD.
!
!    Input, integer ( kind = 4 ) N, the order of the matrix.
...
!    the main diagonal.  0 <= ML < N, 0 <= MU < N.
!
!    Input, integer ( kind = 4 ) IPVT(N), the pivot vector from SGBCO or SGBFA.
!
...
!  Parameters:
!
!    Input, real ( kind = 4 ) ABD(LDA,N), the output from SGBCO or SGBFA.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of the array ABD.
!
!    Input, integer ( kind = 4 ) N, the order of the matrix.
...
!    Input, integer ( kind = 4 ) IPVT(N), the pivot vector from SGBCO or SGBFA.
!
!    Input/output, real ( kind = 4 ) B(N).  On input, the right hand side.
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) A(LDA,N).  On input, a matrix to be
...
!    Output, integer ( kind = 4 ) IPVT(N), the pivot indices.
!
!    Output, real ( kind = 4 ) RCOND, an estimate of the reciprocal
!    condition number of A.
!
!    Output, real ( kind = 4 ) Z(N), a work vector whose contents are usually
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) A(LDA,N), on input, the  LU factor
!    information, as output by SGECO or SGEFA.  On output, the inverse
!    matrix, if requested.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of the array A.
!
...
!
    do k = n-1, 1, -1
      ...
      l = ipvt(k)
      if ( l /= k ) then
        call sswap ( n, a(1:n,k), 1, a(1:n,l), 1 )
      end if
...
!
!    Input/output, real ( kind = 4 ) C(N), contains the subdiagonal of the
!    tridiagonal matrix in entries C(2:N).  On output, C is destroyed.
!
!    Input/output, real ( kind = 4 ) D(N).  On input, the diagonal of the
!    matrix.  On output, D is destroyed.
!
!    Input/output, real ( kind = 4 ) E(N), contains the superdiagonal of the
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) ABD(LDA,N).  On input, the matrix to be
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, integer ( kind = 4 ) M, the number of diagonals above the main
!    diagonal.
!
!    Output, real ( kind = 4 ) RCOND, an estimate of the reciprocal condition
!    of A.  For the system A*X = B, relative perturbations in A and B of size
...
!  Parameters:
!
!    Input, real ( kind = 4 ) ABD(LDA,N), the output from SPBCO or SPBFA.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of the array ABD.
!
!    Input, integer ( kind = 4 ) N, the order of the matrix.
...
!    diagonal.
!
!    Output, real ( kind = 4 ) DET(2), the determinant of the original
!    matrix in the form
!      determinant = DET(1) * 10.0**DET(2)
...
!*****************************************************************************80
!
!! SPOCO factors a real SPD matrix and estimates its condition.
!
!  Discussion:
!
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) A(LDA,N).  On input, the symmetric
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) A(LDA,N).  On input, the output A from
...
!    Input, integer ( kind = 4 ) N, the order of the matrix A.
!
!    Output, real ( kind = 4 ) DET(2), the determinant of A or of X'*X
!    if requested.
!      determinant = DET(1) * 10.0**DET(2)
!    with 1.0E+00 <= DET(1) < 10.0E+00 or DET(1) == 0.0E+00.
!
...
!  Parameters:
!
!    Input, real ( kind = 4 ) A(LDA,N), the output from SPOCO or SPOFA.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of the array A.
!
!    Input, integer ( kind = 4 ) N, the order of the matrix.
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) AP((N*(N+1))/2).  On input, the packed
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) AP((N*(N+1))/2).  On input, the output
!    from SPPCO or SPPFA.  On output, the upper triangular half of the
...
!*****************************************************************************80
!
!! SPPSL solves a real SPD system factored by SPPCO or SPPFA.
!
!  Discussion:
!
!    To compute inverse(A) * C where C is a matrix with P columns
...
!  Parameters:
!
!    Input, real ( kind = 4 ) AP((N*(N+1))/2), the output from SPPCO or SPPFA.
!
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 4 ) B(N).  On input, the right hand side.
...
!
!    Input, real ( kind = 4 ) E(N), the offdiagonal of the tridiagonal matrix in
!    entries E(1:N-1).
!
!    Input/output, real ( kind = 4 ) B(N).  On input, the right hand side.
!    On output, the solution.
  ...
  if ( job /= 0 ) then
    ...
    do j = 1, p
      ...
      if ( swapj ) then
        ...
        if ( j /= pl ) then
          call sswap ( n, a(1:n,pl), 1, a(1:n,j), 1 )
        end if
    ...
    do j = p, 1, -1
      ...
      if ( jpvt(j) < 0 ) then
        ...
        if ( j /= pu ) then
          call sswap ( n, a(1:n,pu), 1, a(1:n,j), 1 )
          jp = jpvt(pu)
          jpvt(pu) = jpvt(j)
          jpvt(j) = jp
        end if
...
!
    if ( pl <= l .and. l < pu ) then
      ...
      if ( maxj /= l ) then
        call sswap ( n, a(1:n,l), 1, a(1:n,maxj), 1 )
        qraux(maxj) = qraux(l)
        work(maxj) = work(l)
        jp = jpvt(maxj)
        jpvt(maxj) = jpvt(l)
...
!*****************************************************************************80
!
!! SSIDI: determinant, inertia and inverse of a real symmetric matrix.
!
!  Discussion:
!
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) A(LDA,N).  On input, the output from SSIFA.
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, integer ( kind = 4 ) KPVT(N), the pivot vector from SSIFA.
!
!    Output, real ( kind = 4 ) DET(2), the determinant of the original matrix,
!    if requested.
!      determinant = DET(1) * 10.0**DET(2)
...
!
      ...
      if ( ks /= k ) then
        ...
        call sswap ( ks, a(1:ks,ks), 1, a(1:ks,k), 1 )
...
!
      ...
      if ( swap ) then
        ...
        call sswap ( imax, a(1:imax,imax), 1, a(1:imax,k), 1 )
...
!
      ...
      if ( swap ) then
        ...
        call sswap ( imax, a(1:imax,imax), 1, a(1:imax,k-1), 1 )
...
!  Parameters:
!
!    Input, real ( kind = 4 ) A(LDA,N), the output from SSIFA.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of the array A.
!
!    Input, integer ( kind = 4 ) N, the order of the matrix.
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) AP((N*(N+1))/2).  On input, the packed form
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) AP((N*(N+1))/2).  On input, the output from
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, integer ( kind = 4 ) KPVT(N), the pivot vector from SSPFA.
!
!    Output, real ( kind = 4 ) DET(2), the determinant of the original matrix,
!    if requested.
!      determinant = DET(1) * 10.0**DET(2)
...
!
        ...
        if ( 2 <= k ) then
          ...
          ap(kk) = ap(kk) + sdot ( k-1, work, 1, ap(ik+1), 1 )
...
!
      ...
      if ( ks /= k ) then
        ...
        iks = ( ks * ( ks - 1 ) ) / 2
        call sswap ( ks, ap(iks+1:iks+ks), 1, ap(ik+1:ik+ks), 1 )
        ksj = ik + ks
...
!  Perform an interchange.
!
          call sswap ( imax, ap(im+1:im+imax), 1, ap(ik+1:ik+imax), 1 )
          imj = ik + imax
...
!
        if ( swap ) then
          ...
          call sswap ( imax, ap(im+1:im+imax), 1, ap(ikm1+1:ikm1+imax), 1 )
          imj = ikm1 + imax
...
!  Parameters:
!
!    Input, real ( kind = 4 ) AP((N*(N+1))/2), the output from SSPFA.
!
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, integer ( kind = 4 ) KPVT(N), the pivot vector from SSPFA.
...
subroutine ssvdc ( a, lda, m, n, s, e, u, ldu, v, ldv, work, job, info )
...
!  Parameters:
!
!    Input/output, real ( kind = 4 ) A(LDA,N).  On input, the M by N
...
!
!    Output, real ( kind = 4 ) S(MM), where MM = max(M+1,N).  The first
!    min(M,N) entries of S contain the singular values of A arranged in
!    descending order of magnitude.
!
!    Output, real ( kind = 4 ) E(MM), where MM = max(M+1,N).  Ordinarily
!    contains zeros.  However see the discussion of INFO for exceptions.
!
!    Output, real ( kind = 4 ) U(LDU,K).  If JOBA = 1 then K = M;
...
!
!    Output, real ( kind = 4 ) V(LDV,N), the N by N matrix of right singular
...
!
    if ( l <= nct ) then
      ...
      s(l) = snrm2 ( m-l+1, a(l,l), 1 )

      if ( s(l) /= 0.0E+00 ) then
        if ( a(l,l) /= 0.0E+00 ) then
          s(l) = sign ( s(l), a(l,l) )
        end if
        call sscal ( m-l+1, 1.0E+00 / s(l), a(l,l), 1 )
        a(l,l) = 1.0E+00 + a(l,l)
      end if
...
!
      if ( l <= nct .and. s(l) /= 0.0E+00 ) then
        t = -sdot ( m-l+1, a(l,l), 1, a(l,j), 1 ) / a(l,l)
        call saxpy ( m-l+1, t, a(l,l), 1, a(l,j), 1 )
      end if
!
...
!
    if ( l <= nrt ) then
      ...
      e(l) = snrm2 ( n-l, e(l+1), 1 )

      if ( e(l) /= 0.0E+00 ) then
        if ( e(l+1) /= 0.0E+00 ) then
          e(l) = sign ( e(l), e(l+1) )
        end if
        call sscal ( n-l, 1.0E+00 / e(l), e(l+1), 1 )
        e(l+1) = 1.0E+00 + e(l+1)
      end if
...
!  Apply the transformation.
!
      if ( l + 1 <= m .and. e(l) /= 0.0E+00 ) then

        work(l+1:m) = 0.0E+00

        do j = l+1, n
          call saxpy ( m-l, e(j), a(l+1,j), 1, work(l+1), 1 )
        end do

        do j = l+1, n
          call saxpy ( m-l, -e(j)/e(l+1), work(l+1), 1, a(l+1,j), 1 )
        end do
  ...
  if ( wantu ) then
    ...
    do ll = 1, nct
      ...
      if ( s(l) /= 0.0E+00 ) then
        ...
        do j = l+1, ncu
          t = -sdot ( m-l+1, u(l,l), 1, u(l,j), 1 ) / u(l,l)
          call saxpy ( m-l+1, t, u(l,l), 1, u(l,j), 1 )
        end do
  ...
  if ( wantv ) then
    ...
    do ll = 1, n
      ...
      if ( l <= nrt .and. e(l) /= 0.0E+00 ) then
        ...
        do j = l+1, n
          t = -sdot ( n-l, v(l+1,l), 1, v(l+1,j), 1 ) / v(l+1,l)
          call saxpy ( n-l, t, v(l+1,l), 1, v(l+1,j), 1 )
        end do
...
!
    if ( kase == 1 ) then
      ...
      do kk = l, mm1
        ...
        if ( wantv ) then
          call srot ( n, v(1,k), 1, v(1,mn), 1, cs, sn )
        end if
...
!
      ...
      do k = l, mn
        ...
        e(k) = cs * e(k)
        if ( wantu ) then
          call srot ( m, u(1,k), 1, u(1,l-1), 1, cs, sn )
        end if
...
!
      ...
      sl = s(l) / scale
      el = e(l) / scale
      b = ( ( smm1 + sm ) * ( smm1 - sm ) + emm1 * emm1 ) / 2.0E+00
      c = sm  * sm * emm1 * emm1
      shift = 0.0E+00
...
!
      ...
      do k = l, mm1
        ...
        if ( wantv ) then
          call srot ( n, v(1,k), 1, v(1,k+1), 1, cs, sn )
        end if
        ...
        if ( wantu .and. k < m ) then
          call srot ( m, u(1,k), 1, u(1,k+1), 1, cs, sn )
        end if
...
!
      do
        ...
        if ( wantv .and. l < n ) then
          call sswap ( n, v(1:n,l), 1, v(1:n,l+1), 1 )
        end if

        if ( wantu .and. l < m ) then
          call sswap ( m, u(1:m,l), 1, u(1:m,l+1), 1 )
        end if
...
!  Parameters:
!
!    Input, real ( kind = 4 ) T(LDT,N), the triangular matrix.  The zero
...
!  Parameters:
!
!    Input, real ( kind = 4 ) T(LDT,N), the matrix of the system.  The zero
!    elements of the matrix are not referenced, and the corresponding
!    elements of the array can be used to store other information.
!
!    Input, integer ( kind = 4 ) LDT, the leading dimension of the array T.
