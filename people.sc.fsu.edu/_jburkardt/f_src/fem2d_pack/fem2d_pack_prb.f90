...
!
  ...
  integer ( kind = 4 ), allocatable, dimension ( :, : ) :: element_node
  integer ( kind = 4 ) element_num
  character ( len = 255 ) :: file_name = 'fem2d_pack_prb_q4.eps'
  integer ( kind = 4 ) node
  integer ( kind = 4 ) node_num
  real ( kind = 8 ), allocatable, dimension ( :, : ) :: node_xy
  real ( kind = 8 ) r
  integer ( kind = 4 ) row
  character ( len = 255 ) :: title = 'Grid of Q4 Elements'
...
!
  ...
  integer ( kind = 4 ), parameter :: element_num = 8
  integer ( kind = 4 ), parameter :: element_order = 3
  ...
  integer ( kind = 4 ), dimension ( element_order, element_num ) :: element_node = &
    ...
    6, 3, 5, &
    5, 8, 6, &
    9, 6, 8 /), (/ element_order, element_num /) )
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST135'
  write ( *, '(a)' ) '  MASS_MATRIX_T3 computes the mass matrix for'
  write ( *, '(a)' ) '  a finite element system using T3 elements'
  write ( *, '(a)' ) '  (linear triangles).'

  call mass_matrix_t3 ( node_num, element_num, element_node, node_xy, a )

  call r8mat_print ( node_num, node_num, a, '  The T3 mass matrix:' )
