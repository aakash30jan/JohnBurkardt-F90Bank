...
!
!    Local, integer ( kind = 4 ) NODE_BOUNDARY(NODE_NUM), is
!    0, if a node is an interior node;
!    1, if a node is a Dirichlet boundary node.
!
!    Local, integer ( kind = 4 ) ELEMENT_ORDER, the number of nodes used to 
!    form one element.
!
...
!
!    Local, integer ( kind = 4 ) TIME_STEP_NUM, the number of time steps
!    to take.
!
...
!
  ...
  integer ( kind = 4 ) job
  integer ( kind = 4 ) node
  integer ( kind = 4 ), dimension(node_num) :: node_boundary
...
subroutine adjust_boundary ( node_num, node_xy, node_boundary, ib, time, a, f )
...
!    Input, real ( kind = 8 ) NODE_XY(2,NODE_NUM), the coordinates of nodes.
!
!    Input, integer ( kind = 4 ) NODE_BOUNDARY(NODE_NUM), is
!    0, if a node is an interior node;
!    1, if a node is a Dirichlet boundary node.
!
!    Input, integer ( kind = 4 ) IB, the half-bandwidth of the matrix.
...
!
!    Input, integer ( kind = 4 ) ELEMENT_ORDER, the number of local nodes 
!    per element.
!
!    Input, integer ( kind = 4 ) ELEMENT_NUM, the number of elements.
!
...
!    form one element.
!
!    Input, integer ( kind = 4 ) QUAD2_NUM, the number of points in the 
!    quadrature rule.  This is actually fixed at 13.
!
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
!    Input, real ( kind = 8 ) TIME, the current time.
...
!
  ...
  integer ( kind = 4 ) element_order
  integer ( kind = 4 ) node_num
  integer ( kind = 4 ), parameter :: quad2_num = 13
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes at which
!    a value is desired.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) NX, NY, controls the number of elements along
!    the X and Y directions.  The number of elements will be
!    2 * ( NX - 1 ) * ( NY - 1 ).
!
!    Input, integer ( kind = 4 ) ELEMENT_ORDER, the number of local nodes 
!    per element.
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) NX, NY, the number of elements in the X 
!    and Y directions.
!
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
!    Output, integer ( kind = 4 ) NODE_BOUNDARY(NODE_NUM), is
!    0, if a node is an interior node;
!    1, if a node is a Dirichlet boundary node.
!
  implicit none
  ...
  integer ( kind = 4 ) j
  integer ( kind = 4 ) node
  integer ( kind = 4 ) node_boundary(node_num)
  integer ( kind = 4 ) nx
  integer ( kind = 4 ) ny
...
!
!    Input, integer ( kind = 4 ) ELEMENT, the index of the element which 
!    contains the point.
!
...
!    ELEMENT_NODE(I,J) is the global index of local node I in element J.
!
!    Input, integer ( kind = 4 ) ELEMENT, the index of the element for which
!    the quadrature points are to be computed.
!
!    Input, integer ( kind = 4 ) ELEMENT_NUM, the number of elements.
!
!    Input, integer ( kind = 4 ) ELEMENT_ORDER, the number of nodes used to
...
!
!    Input, integer ( kind = 4 ) TRIANGLE_NODE(6,TRI_NUM), lists, for each
!    triangle, the indices of the points that form the vertices of the triangle.
!
