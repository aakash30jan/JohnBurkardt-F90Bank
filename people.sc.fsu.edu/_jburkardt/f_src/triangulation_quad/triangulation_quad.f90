...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TRIANGULATION_QUAD:'
  write ( *, '(a)' ) '  FORTRAN90 version:'
  write ( *, '(a)' ) '  Estimate an integral over a triangulated region.'
!
...
!
  ...
  if ( node_dim /= 2 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGULATION_QUAD - Fatal error!'
...
!
  ...
  call i4mat_transpose_print_some ( element_order, element_num, &
    element_node, 1, 1, element_order, 5, '  First 5 triangles:' )
!
!  Read the value data.
!
...
!
  allocate ( area(1:element_num) )

  area(1:element_num) = 0.5 * abs ( &
      node_xy(1,element_node(1,1:element_num)) &
    * ( node_xy(2,element_node(2,1:element_num)) &
    - node_xy(2,element_node(3,1:element_num)) ) &
    + node_xy(1,element_node(2,1:element_num)) &
    * ( node_xy(2,element_node(3,1:element_num)) &
    - node_xy(2,element_node(1,1:element_num)) ) &
...
!
  ...
  quad(1:value_dim) = matmul ( &
    ...
    area(1:element_num) ) / 3.0D+00

  write ( *, '(a)' ) ' '
  write ( *, '(a,g14.6)' ) &
    '  Triangulation area is ', sum ( area(1:element_num) )
