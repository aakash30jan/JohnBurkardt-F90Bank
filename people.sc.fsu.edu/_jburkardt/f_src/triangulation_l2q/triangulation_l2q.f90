...
!
  ...
  character ( len = 255 ) :: element_l2q_filename = ' '
  integer ( kind = 4 ), allocatable, dimension ( :, : ) :: element_neighbor
  integer ( kind = 4 ), allocatable, dimension ( :, : ) :: element_node_t3
  integer ( kind = 4 ), allocatable, dimension ( :, : ) :: element_node_t6
  integer ( kind = 4 ) element_num
  integer ( kind = 4 ) element_order_t3
  integer ( kind = 4 ) element_order_t6
  ...
  integer ( kind = 4 ) ii
  integer ( kind = 4 ) iii
  integer ( kind = 4 ) interior_num
  integer ( kind = 4 ) ip1
  integer ( kind = 4 ) k1
  integer ( kind = 4 ) k2
  character ( len = 255 ) :: node_filename = ' '
  character ( len = 255 ) :: node_l2q_filename = ' '
  integer ( kind = 4 ) node_num_t3
  integer ( kind = 4 ) node_num_t6
  real ( kind = 8 ), allocatable, dimension ( :, : ) :: node_xy_t3
  real ( kind = 8 ), allocatable, dimension ( :, : ) :: node_xy_t6
  character ( len = 255 ) prefix

  call timestamp ( )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TRIANGULATION_L2Q'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Read a "linear" T3 triangulation and'
  write ( *, '(a)' ) '  write out a "quadratic" T6 triangulation.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read a dataset of NODE_NUM1 points in 2 dimensions.'
  write ( *, '(a)' ) &
    '  Read an associated triangulation dataset of ELEMENT_NUM '
  write ( *, '(a)' ) '  triangles which uses 3 nodes per element.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Create new nodes which are triangle midpoints,'
  write ( *, '(a)' ) '  generate new node and triangulation data for'
  write ( *, '(a)' ) '  quadratic 6-node elements, and write them out.'
!
...
!
  ...
  node_l2q_filename = trim ( prefix ) // '_l2q_nodes.txt'
  element_l2q_filename = trim ( prefix ) // '_l2q_elements.txt'
!
!  Read the node data.
!
  call r8mat_header_read ( node_filename, dim_num, node_num_t3 )
  ...
  write ( *, '(a,i8)' ) '  Number of points NODE_NUM_T3  = ', node_num_t3

  allocate ( node_xy_t3(1:dim_num,1:node_num_t3) )

  call r8mat_data_read ( node_filename, dim_num, node_num_t3, node_xy_t3 )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read the data in "' &
    // trim ( node_filename ) //'".'

  call r8mat_transpose_print_some ( dim_num, node_num_t3, node_xy_t3, 1, 1, &
    5, 5, '  5 by 5 portion of data read from file:' )
!
!  Read the element data.
!
  call i4mat_header_read ( element_filename, element_order_t3, element_num )

  if ( element_order_t3 /= 3 ) then
    write ( *, * ) ' '
    write ( *, '(a)' ) 'TRIANGULATION_L2Q - Fatal error!'
    write ( *, '(a)' ) '  Data is not for a 3-node triangulation.'
    stop
  end if
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Element order = ', element_order_t3
  write ( *, '(a,i8)' ) '  Number of elements ELEMENT_NUM  = ', element_num

  allocate ( element_node_t3(1:element_order_t3,1:element_num) )

  call i4mat_data_read ( element_filename, element_order_t3, element_num, &
    element_node_t3 )
  ...
  call i4mat_transpose_print_some ( element_order_t3, element_num, &
    element_node_t3, 1, 1, 3, 10, '  First 10 elements:' )
!
!  Detect and correct 0-based indexing.
!
  call mesh_base_one ( node_num_t3, element_order_t3, element_num, &
    element_node_t3 )
!
!  Determine the number of midside nodes that will be added.
!
  call triangulation_order3_boundary_edge_count ( element_num, &
    element_node_t3, boundary_num )

  interior_num = ( 3 * element_num - boundary_num ) / 2
  edge_num = interior_num + boundary_num
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Number of midside nodes to add = ', edge_num

  node_num_t6 = node_num_t3 + edge_num
!
!  Allocate space.
!
  element_order_t6 = 6

  allocate ( element_neighbor(1:3,element_num) )
  allocate ( node_xy_t6(1:dim_num,1:node_num_t6) )
  allocate ( element_node_t6(1:element_order_t6,1:element_num) )
!
!  Build the element neighbor array.
!
  call triangulation_neighbor_triangles ( element_order_t3, element_num, &
    element_node_t3, element_neighbor )
...
!
  element_node_t6(1:3,1:element_num) = element_node_t3(1:3,1:element_num)
  element_node_t6(4:6,1:element_num) = -1
  node_xy_t6(1:dim_num,1:node_num_t3) = node_xy_t3(1:dim_num,1:node_num_t3)

  node_num_t6 = node_num_t3

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Generate midside nodes'
  write ( *, '(a)' ) ' '
...
!
      iii = i4_wrap ( i + 2, 1, 3 )
      element2 = element_neighbor(iii,element)
!
!  Perhaps the extra node on this side was already created when
!  we dealt with a lower-numbered element.
!
      if ( 0 < element2 .and. element2 < element ) then
        cycle
      end if
      ...
      k1 = element_node_t6(i,element)
      k2 = element_node_t6(ip1,element)

      node_num_t6 = node_num_t6 + 1
      node_xy_t6(1:dim_num,node_num_t6) = 0.5D+00 &
        * ( node_xy_t3(1:dim_num,k1) + node_xy_t3(1:dim_num,k2) )
      write ( *, '(i8,2x,2g14.6)' ) &
        node_num_t6, node_xy_t6(1:dim_num,node_num_t6)
      element_node_t6(3+i,element) = node_num_t6   
...
!
          ...
          if ( element_neighbor(iii,element2) == element ) then
            element_node_t6(ii+3,element2) = node_num_t6
          end if
        end do
      end if
  ...
  call i4mat_transpose_print ( element_order_t6, element_num, element_node_t6, &
    '  ELEMENT_NODE_T6' )

  call r8mat_transpose_print ( dim_num, node_num_t6, node_xy_t6, &
    '  NODE_XY_T6:' )
!
!  Write out the node and element data for the quadratic mesh
!
  call r8mat_write ( node_l2q_filename, dim_num, node_num_t6, node_xy_t6 )

  call i4mat_write ( element_l2q_filename, element_order_t6, element_num, &
    element_node_t6 )
!
!  Free memory.
!
  deallocate ( element_neighbor )
  deallocate ( element_node_t3 )
  deallocate ( element_node_t6 )
  deallocate ( node_xy_t3 )
  deallocate ( node_xy_t6 )
!
!  Terminate.
!
  write ( *, '(a)' ) ' '
...
!
!    Output, integer ( kind = 4 ) BOUNDARY_EDGE_NUM, the number of 
!    boundary edges.
!
  implicit none
