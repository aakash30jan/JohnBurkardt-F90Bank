...
!
  ...
  integer ( kind = 4 ) point_num_1d
  integer ( kind = 4 ) point_num_1d2
  ...
  real ( kind = 8 ), allocatable, dimension ( : ) :: r_1d
  character ( len = 255 ) string
  real ( kind = 8 ), allocatable, dimension ( : ) :: w
  real ( kind = 8 ), allocatable, dimension ( : ) :: w_1d
  real ( kind = 8 ), allocatable, dimension ( :, : ) :: x
  real ( kind = 8 ), allocatable, dimension ( : ) :: x_1d
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) POINT_NUM_1D, the order of the 1D rule.
!
!    Input, real ( kind = 8 ) X_1D(POINT_NUM_1D), the points of the 1D rule.
!
!    Input, real ( kind = 8 ) W_1D(POINT_NUM_1D), the weights of the
!    1D rule.
!
!    Input, real ( kind = 8 ) R_1D(2), the extreme points that define
...
!
!    Output, real ( kind = 8 ) X(DIM_NUM,POINT_NUM), the points of the rule.
!
!    Output, real ( kind = 8 ) W(POINT_NUM), the weights of the rule.
!
!    Output, real ( kind = 8 ) R(DIM_NUM,2), the extreme points 
...
!
  ...
  real ( kind = 8 ) w_1d(point_num_1d)
  real ( kind = 8 ) x(dim_num,point_num)
  real ( kind = 8 ) x_1d(point_num_1d)
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) POINT_NUM_1D, the number of points in the 
!    1D rule.
!
!    Input, integer ( kind = 4 ) DIM_NUM, the spatial dimension.
!
