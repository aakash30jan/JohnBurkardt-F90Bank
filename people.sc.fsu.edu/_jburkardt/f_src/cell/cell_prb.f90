...
!
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'TEST01'
  write ( *, '(a)' ) '  Use a real cell array (vector of vectors) to store rows 3:7'
...
!
  ...
  integer ( kind = 4 ), allocatable :: roff(:)
  integer ( kind = 4 ) row
  integer ( kind = 4 ), allocatable :: vn(:)

  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'TEST02'
  write ( *, '(a)' ) '  Use an integer cell array (vector of vectors) to store rows 3:7'
  write ( *, '(a)' ) '  of Pascal''s triangle.'
...
!
  ...
  do row = 1, 7
    ...
    if ( 3 <= row ) then
      i = row - 2
      call i4cvv_rset ( mn, a, m, roff, i, ai )
    end if
...
!  Print the cell array.
!
  call i4cvv_print ( mn, a, m, roff, '  Rows 3:7 of Pascal''s Triangle:' )
!
!  Retrieve the entry from cell array row 3, column 4:
!
  i = 3
  j = 4
  call i4cvv_iget ( mn, a, m, roff, i, j, aij )
  write ( *, '(a)' ) ' '
  write ( *, '(a,i1,a,i1,a,g14.6)' ) '  A(', i, ',', j, ') = ', aij
!
!  Retrieve row 4:
!
  i = 4
  call i4cvv_rget ( mn, a, m, roff, i, ai )
  nv = roff(i+1) - roff(i)
  call i4vec_transpose_print ( nv, ai, '  A(4,*):' )
!
!  Retrieve a list of entries.
!
  nn = 4
  ...
  in = (/ 1, 2, 5, 5 /)
  jn = (/ 2, 3, 4, 8 /)
  call i4cvv_nget ( mn, a, m, roff, nn, in, jn, vn )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Retrieve an arbitrary list of items:'
  write ( *, '(a)' ) ' '
  do i = 1, nn
    write ( *, '(a,i1,a,i1,a,i6)' ) '  A(', in(i), ',', jn(i), ') = ', vn(i)
  end do
!
!  Free memory.
!
