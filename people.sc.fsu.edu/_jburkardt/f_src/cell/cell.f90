...
!    1 <= J <= NR(I).
!
!    Output, integer ( kind = 4 ) K, the index of item J in row I.
!
  implicit none
...
!    1 <= JN(*) <= NR(IN(*)).
!
!    Input, integer ( kind = 4 ) DVN(NN), the increments of 
!    items A(IN(*),JN(*)).
!
  implicit none
...
!    1 <= JN(*) <= NR(IN(*)).
!
!    Output, integer ( kind = 4 ) KN(NN), the indices of items A(IN(*),JN(*)).
!
  implicit none
...
!    1 <= I <= M.
!
!    Output, integer ( kind = 4 ) K, the index of row I.
!
  implicit none
