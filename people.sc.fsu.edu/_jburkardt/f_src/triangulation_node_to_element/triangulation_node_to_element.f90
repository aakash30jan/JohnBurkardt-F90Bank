...
!
  ...
  real ( kind = 8 ), allocatable, dimension ( :, : ) :: element_value
  character ( len = 255 ) element_value_filename
  integer ( kind = 4 ) i
  integer ( kind = 4 ) iarg
  ...
  call timestamp ( )
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'TRIANGULATION_NODE_TO_ELEMENT:'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Average nodal data to create element data.'
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  This program expects three files:'
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  * prefix_nodes.txt,    node coordinates,'
  write ( *, '(a)' ) '  * prefix_elements.txt, indices of nodes forming elements,'
  write ( *, '(a)' ) '  * prefix_values.txt,   data values at nodes.'
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  It creates:'
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  * prefix_element_values.txt, averaged data values at elements.'
!
...
!
  ...
  element_value_filename = trim ( prefix ) // '_element_values.txt'
!
!  Read the node data.
!
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read the header of "' // trim ( value_filename ) //'".'
  write ( *, '(a)' ) ' '
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read the data in "' // trim ( value_filename ) //'".'

  call r8mat_transpose_print_some ( value_dim, value_num, value, 1, 1, &
    value_dim, 5, '  First 5 node values:' )
!
!  Detect and correct 0-based indexing.
!
  call mesh_base_one ( node_num, element_order, element_num, element_node )
!
!  Create the element_values data.
!
  allocate ( element_value(1:value_dim,1:element_num) )

  element_value(1:value_dim,1:element_num) = 0.0D+00
  ...
  element_value(1:value_dim,1:element_num) = &
    element_value(1:value_dim,1:element_num) / real ( element_order, kind = 8 )
!
!  Write out the file.
!
  call r8mat_write ( element_value_filename, value_dim, element_num, &
    element_value )

  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  Element values written to "' &
    // trim ( element_value_filename ) // '"'
!
!  Free memory.
!
  deallocate ( element_node )
  deallocate ( element_value )
  deallocate ( node_xy )
  deallocate ( value )
!
!  Terminate.
