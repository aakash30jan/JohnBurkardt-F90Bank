...
!    Input, integer ( kind = 4 ) NPROB, the problem number.
!
!    Output, integer ( kind = 4 ) M, information defining the number of 
!    equations for the system.  If M is positive, then its value is usually
!    the only one appropriate for the problem.  If M is negative,
!    then any value of M greater than or equal to N is appropriate.
!
!    Output, integer ( kind = 4 ) N, information defining the number of 
!    variables for the system.  If N is positive, then its value is usually
