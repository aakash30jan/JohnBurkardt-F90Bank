...
!
!    Output, real ( kind = 8 ) TTIME, the CPU time in seconds 
!    required for the matrix multiplications.
!
  implicit none
...
!  Parameters:
!
!    Output, complex ( kind = 4 ) A(LDA,N), B(LDA,N), C(LDA,N), the three 
!    matrices used in the multiplication.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension used for arrays.
!
!    Input, integer ( kind = 4 ) N, the number of rows and columns 
...
!    Output, logical MFLOPS_SHOW, is TRUE if the MegaFLOP rate is to be shown.
!
!    Output, integer ( kind = 4 ) N, the number of rows and columns 
!    in the matrices.
!
!    Output, integer ( kind = 4 ) NHI, the maximum value of N to use.
!
!    Output, integer ( kind = 4 ) NINC, the additive increment to use, if 
!    additive steps are being taken.
!
!    Output, integer ( kind = 4 ) NLO, the smallest value of N to use.
!
!    Output, integer ( kind = 4 ) NMULT, the multiplier to use, if 
!    multiplicative steps are being taken.
!
!    Output, logical NOSHOW, is TRUE if the number of operations is
!    to be shown.
!
...
!  Parameters:
!
!    Workspace, integer ( kind = 4 ) A(LDA,N), B(LDA,N), C(LDA,N), 
!    the three matrices used in the multiplication.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension used for arrays.
!
!    Input, integer ( kind = 4 ) N, the number of rows and columns.
...
!
!    Input/output, integer ( kind = 4 ) LDA, the leading dimension used 
!    for arrays.  Automatically reset to N if it is less than N.
!
...
!    Input, integer ( kind = 4 ) LDA, the leading dimension used for arrays.
!
!    Output, integer ( kind = 4 ) N, the new value for the number of rows 
!    and columns to use in the matrices.
!
!    Output, integer ( kind = 4 ) NHI, the maximum value of N to use.
!
!    Output, integer ( kind = 4 ) NINC, the additive increment to use, if
!    additive steps are being taken, or 0.
!
!    Output, integer ( kind = 4 ) NLO, the smallest value of N to use.
!
!    Output, integer ( kind = 4 ) NMULT, the multiplier to use, if 
!    multiplicative steps are being taken, or 0.
!
  implicit none
...
!
!    Input/output, integer ( kind = 4 ) N, the number of rows and columns 
!    in the matrices.  This routine modifies the value of N as appropriate.
!
!    Input, integer ( kind = 4 ) NHI, the maximum value of N to use.
!
!    Input, integer ( kind = 4 ) NINC, the additive increment to use, if 
!    additive steps are being taken.
!
...
!  Parameters:
!
!    Workspace, real ( kind = 4 ) A(LDA,N), B(LDA,N), C(LDA,N), the matrices
!    used in the multiplication.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension used for arrays.
!
...
!    Input, integer ( kind = 4 ) NREP, the number of repetitions to carry out.
!
!    Output, real ( kind = 8 ) TTIME, the CPU time in seconds required
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the "logical" number of items in the 
!    vectors.
!
!    Input, real ( kind = 4 ) SA, the multiplier.
!
!    Input, real ( kind = 4 ) SX(*), a vector, a multiple of which is to be 
!    added to SY.
!
!    Input, integer ( kind = 4 ) INCX, the increment in SX between the 
!    successive elements that we will use.
!
!    Input/output, real ( kind = 4 ) SY(*), a vector, to which is to be added 
!    SA times an entry of SX.
!
!    Input, integer ( kind = 4 ) INCY, the increment in SY between the 
!    successive elements that we will use.
!
  implicit none
