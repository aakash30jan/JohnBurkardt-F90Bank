...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'LEGENDRE_EXACTNESS'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) &
    '  Investigate the polynomial exactness of a Gauss-Legendre'
  write ( *, '(a)' ) '  quadrature rule by integrating '
  write ( *, '(a)' ) &
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'LEGENDRE_EXACTNESS: User input:'
...
!
  ...
  if ( dim_num /= 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'LEGENDRE_EXACTNESS - Fatal error!'
...
!
  ...
  if ( order == 1 ) then
    ...
    w2(1:order) = 0.0D+00
    w2(1:order-1) = w2(1:order-1) + 1.0D+00 / real ( order - 1, kind = 8 )
    w2(2:order)   = w2(2:order)   + 1.0D+00 / real ( order - 1, kind = 8 )

    do i = 1, order
      x2(i) = ( real ( order - i,     kind = 8 ) * ( -1.0D+00 ) &
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  A Gauss-Legendre rule would be able to exactly'
  write ( *, '(a,i8)' ) &
    '  integrate monomials up to and including degree = ', 2 * order - 1
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) &
    '          Error                      Error           Degree'
  write ( *, '(a)' ) '         (This rule)                (Trapezoid)'
  write ( *, '(a)' ) ' '
  ...
  do degree = 0, degree_max
    ...
    call legendre_monomial_quadrature ( expon, order, w, x, quad_error )

    call legendre_monomial_quadrature ( expon, order, w2, x2, quad_error2 )

    write ( *, '(2x,f24.16,3x,f24.16,2x,i2)' ) &
...
subroutine legendre_monomial_quadrature ( expon, order, w, x, quad_error )
...
!  Get the exact value of the integral.
!
  call legendre_integral ( expon, exact )
!
