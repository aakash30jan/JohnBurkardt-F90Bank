...
!
!    Input, integer ( kind = 4 ) PTHLEN(KPATHS+3); the shortest path lengths
!    are stored in PTHLEN(2), PTHLEN(3), ..., PTHLEN(KPATHS+1).
!
!    Input, integer ( kind = 4 ) ARCDIR(NNODE); array of shortest path tree, 
!    as computed by KSHOT2.
!
!    Input, integer ( kind = 4 ) QUFIRP(KPATHS+3); description of the first 
!    path in a queue, as computed by KSHOT2.
!
!    Input, integer ( kind = 4 ) QUNXTP(KPATHS+3); next entry in a queue, 
!    as computed by KSHOT2.
!
!    Input, integer ( kind = 4 ) CROSAR(MAXQUE); cross edge of the path, as 
!    computed by KSHOT2.
!
!    Input, integer ( kind = 4 ) NEXTRD(MAXQUE); next entry of the pool storage,
!    as computed by KSHOT2.
!
...
!
!    Output, integer ( kind = 4 ) ARCNOD(NNODE); the edges of the K-th shortest
!    path are stored in ARCNOD(1) through ARCNOD(NUMP).
!
  implicit none
...
!    and FALSE otherwise.
!
!    Output, integer ( kind = 4 ) HCYCLE(NNODE); the nodes of the hamiltonian
!    cycle are stored in order in HCYCLE.
!
...
!
!    Input, integer ( kind = 4 ) ISORCE, the starting point for the shortest
!    paths.
!
...
!
!    Input, integer ( kind = 4 ) INODE(NEDGE), JNODE(NEDGE).  The I-th edge 
!    is directed from INODE(I) to JNODE(I).  It is assumed that the array JNODE
!    is already sorted in nondecreasing order.
!
...
!*****************************************************************************80
!
!! DIGRAPH_ARC_KSHORT2: KPATHS shortest distinct path lengths, no repeat nodes.
!
!  Discussion:
!
...
!    is directed from INODE(I) to JNODE(I).
!
!    Input, integer ( kind = 4 ) ARCLEN(NEDGE); ARCLEN(I) is the length of
!    the I-th edge.
!
...
!
!    Input, integer ( kind = 4 ) ISORCE, ISINK; the paths to be found must
!    go from node ISORCE to node ISINK.
!
...
!
!    Output, integer ( kind = 4 ) IPATHS, the actual number of shortest paths
!    found by KSHOT2; IPATHS <= KPATHS.
!
!    Output, integer ( kind = 4 ) PTHLEN(KPATHS+3); the shortest path lengths
!    are stored in PTHLEN(2), PTHLEN(3), ..., PTHLEN(KPATHS+1).
!
...
!*****************************************************************************80
!
!! DIGRAPH_ARC_MINEQV finds minimal equivalent of a strongly connected digraph.
!
!  Warning:
!
...
!
!    Input, integer ( kind = 4 ) INODE(NEDGE), JNODE(NEDGE); the I-th edge of 
!    the digraph is directed from node INODE(I) to node JNODE(I).  It is 
!    assumed that the input digraph is strongly connected.
!
...
!*****************************************************************************80
!
!! DIGRAPH_ARC_PRT_PATH: MAXPTH number of K shortest paths between two nodes.
!
!  Discussion:
!
...
!    Input, integer ( kind = 4 ) NEDGE, the number of edges.
!
!    Input, integer ( kind = 4 ) NPMAX, the maximum number of nodes in a 
!    possible path which allows repeated nodes.
!
!    Input, integer ( kind = 4 ) K, the number of shortest paths.
!
!    Input, integer ( kind = 4 ) MAXPTH, the maximum number of paths to 
!    be generated.
!
!    Input, integer ( kind = 4 ) ISORCE, ISINK; the paths to be generated will 
!    go from node ISORCE to ISINK.  ISORCE and ISINK may be equal.
!
!    Input, integer ( kind = 4 ) INODE(NEDGE), JNODE(NEDGE).  The I-th edge
!    is directed from INODE(I) to JNODE(I).  It is assumed that the array JNODE
!    is already sorted in nondecreasing order.
...
!    Input, integer ( kind = 4 ) NEDGE, the number of edges.
!
!    Input, integer ( kind = 4 ) IROOT, the base node, from which distances to
...
!
!    Output, integer ( kind = 4 ) ITREE(NNODE-1), JTREE(NNODE-1), the arcs in 
...
!
!    Output, integer ( kind = 4 ) NUMCOMP, the number of strongly connected 
!    components of the digraph
!
!    Output, integer ( kind = 4 ) COMP(NNODE), contains the component of 
!    each node.
!
  implicit none
...
!
!    Input, integer ( kind = 4 ) INODE(NEDGE), JNODE(NEDGE); the I-th edge
!    of the digraph extends from node INODE(I) to JNODE(I).
!
...
!*****************************************************************************80
!
!! DIGRAPH_DIST_ALLPATH finds shortest paths for all pairs of nodes in digraph.
!
!  Discussion:
!
...
!
!    Output, integer ( kind = 4 ) NEXT(NNODE,NNODE); NEXT(I,J) is the 
!    next-to-last node in the shortest path from node I to node J.  This can 
!    be used to trace the shortest path for every pair of nodes.
!
  implicit none
...
!*****************************************************************************80
!
!! DIGRAPH_DIST_FMIN stores K such that DISTAB(K) = LITTLE and DISTAB(J) = KEY.
!
!  Discussion:
!
...
!
!    The algorithm requires NNODE**3/2 additions and NNODE**3 comparisons.
!
!  Modified:
!
!    23 July 2000
...
!    for all I.
!
!    Input, integer ( kind = 4 ) NDIM, the row dunensdon of matrix DIST exactly
!    as specifled in the dimension statement of the calling program.
!
!    Input, integer ( kind = 4 ) IROOT, the node from which all distances will
!    be measured.
!
...
!*****************************************************************************80
!
!! DIGRAPH_DIST_SHORTP finds shortest path from ISORCE to ISINK in a digraph.
!
!  Discussion:
!
...
!    for all I.
!
!    Input, integer ( kind = 4 ) NDIM, the row dimension of matrix DIST exactly
!    as specified in the dimension statement of the calling program.
!
!    Input, integer ( kind = 4 ) ISORCE, ISINK, the specified source and 
!    destination node.
!
!    Output, integer ( kind = 4 ) NUMNOD, the number of nodes in the shortest
!    path from ISORCE to ISINK.
!
!    Output, integer ( kind = 4 ) PATH(NNODE); the nodes of the shortest path
!    are stored in entries 1 through NUMNOD of PATH.
!
...
!    JNODE(I) are the two end nodes of the I-th edge in the input graph, which
!    is not necessarily connected.
!
!    Output, integer ( kind = 4 ) CLIQ(NNODE); each time the output WRITE 
!    statement in CLIQUE is executed, a clique CLIQ(I), I = 1, 2,. .., CLIQ_NUM,
!    is generated, where CLIQ_NUM is a local variable in CLIQUE.
!
...
!
!    Input, integer ( kind = 4 ) INODE(NEDGE), JNODE(NEDGE); the I-th edge 
!    of the graph extends from node INODE(I) to JNODE(I).
!
!    Output, integer ( kind = 4 ) NCOLOR, the number of colors used.
!
!    Output, integer ( kind = 4 ) COLOR(NNODE), the colors assigned to 
!    each node.
!
  implicit none
...
!    starts at node INODE(I) and ends at node JNODE(I).
!
!    Output, integer ( kind = 4 ) CPOLY1(NNODE), CPOLY2(NNODE), CPOLY3(NNODE),
!    the coefficients of the chromatic polynomial, in Alternating Sign Standard
...
!*****************************************************************************80
!
!! GRAPH_ARC_CONECT finds bridges, blocks and cut nodes of an undirected graph.
!
!  Discussion:
!
...
!    Input, integer ( kind = 4 ) NEDGE, the number of edges.
!
!    Input, integer ( kind = 4 ) INODE(NEDGE), JNODE(NEDGE); INODE(I), 
!    JNODE(I) are the end nodes of the I-th edge in the input graph.
!
!    Input, integer ( kind = 4 ) IROOT, a specified node for the root of 
!    the tree.
!
...
!
!    Output, integer ( kind = 4 ) CUTNOD(NNODE); during execution of the 
!    routine, CUTNOD(I) is the unique predecessor of node I in the tree.  
!    On output, the cut nodes are stored in
!      CUTNOD(I), I = 1, 2, ..., NCUT.
!
!    Output, integer ( kind = 4 ) BRIDGE(NEDGE); during execution of the 
!    routine, BRIDGE(I) is the end node of the I-th edge in the forward star 
!    representation of the graph. On output, if BRIDGE(I) = 1, then 
!    (INODE(I),JNODE(J)) is a bridge, and if BRIDGE(I) = 0 then the I-th 
...
!
!    Input, integer ( kind = 4 ) INODE(NEDGE), JNODE(NEDGE); INODE(I), JNODE(I)
!    are the end nodes of the I-th edge in the graph.
!
...
!    JNODE(I) are the end nodes of the I-th edge in the input graph.
!
!    Output, integer ( kind = 4 ) NUMCYC, the number of independent cycles 
!    in the graph.
!
!    Output, integer ( kind = 4 ) NUMCMP, the number of components of the graph.
!
!    Output, integer ( kind = 4 ) NCYC(NNODE); each time the output WRITE 
!    statement in FCYCLE is executed, a fundamental cycle
!      NCYC(i), i = 1, 2,. . ., LEN
...
!    must be at least 2.
!
!    Output, integer ( kind = 4 ) INODE((NNODE*K)/2), JNODE((NNODE*K)/2); 
!    INODE(I), JNODE(I) are the end nodes of the I-th edge in the graph.
!
  implicit none
...
!    the graph extends from node INODE(I) to JNODE(I).
!
!    Output, integer ( kind = 4 ) PAIR(NNODE), describes the matching.  Node I 
!    is matched with node PAIR(I).  If PAIR(I) is 0, then node I is
!    unmatched.
!
...
!*****************************************************************************80
!
!! GRAPH_ARC_MINTR2: minimum spanning tree of a graph, using an edge array.
!
!  Discussion:
!
...
!    NTREE will have the value NNODE-1, the number of edges in the spanning 
!    tree.  Otherwise, NTREE will be the number of edges in the minimum spanning
!    forest.
!
...
!    Input, integer ( kind = 4 ) M, the number of edges.
!
!    Input, integer ( kind = 4 ) INODE(M), JNODE(M), the pairs of nodes that
...
!*****************************************************************************80
!
!! GRAPH_ARC_TO_STAR: forward star representation of an undirected graph.
!
!  Modified:
!
!    04 September 1999
...
!    of nodes that define the edges of the tree.
!
  implicit none
