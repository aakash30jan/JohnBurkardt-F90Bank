...
!
  ...
  integer ( kind = 4 ) it
  integer ( kind = 4 ) ln2
  integer ( kind = 4 ), parameter :: ln2_max = 24
  real ( kind = 8 ) mflops
  integer ( kind = 4 ) n
  integer ( kind = 4 ) nits
...
!
        ...
        flops = 2.0D+00 * real ( nits, kind = 8 ) &
          * ( 5.0D+00 * real ( n, kind = 8 ) * real ( ln2, kind = 8 ) )

        mflops = flops / 1.0D+06 / wtime

        write ( *, '(2x,i12,2x,i8,2x,g14.6,2x,g12.4,2x,g12.4,2x,g12.4)' ) &
          n, nits, error, wtime, wtime / real ( 2 * nits, kind = 8 ), mflops
...
!
  ...
  t = mod ( 16807.0D+00 * real ( seed, kind = 8 ), d2 )
  seed = t
  ggl = real ( ( t - 1.0D+00 ) / ( d2 - 1.0D+00 ), kind = 8 )
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the size of the arrays.
!
  implicit none
