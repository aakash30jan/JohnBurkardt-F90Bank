...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the polynomial exactness of a quadrature'
  write ( *, '(a)' ) '  rule by integrating all monomials of a given degree'
  write ( *, '(a)' ) '  over the [0,1] interval.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) &
...
!
  if ( 2 <= arg_num ) then
    ...
    write ( *, '(a)' ) 'INT_EXACTNESS:'
    write ( *, '(a)' ) &
      '  Please enter DEGREE_MAX, the maximum monomial degree to check.'
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS: User input:'
...
!
  ...
  if ( .false. ) then
    if ( dim_num /= 1 ) then
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'INT_EXACTNESS - Fatal error!'
...
!
  write ( *, '(a)'       ) ' '
  write ( *, '(a)'       ) '  The quadrature rule to be tested:'
  write ( *, '(a,i8)'    ) '  ORDER = ', order
  write ( *, '(a)'       ) ' '
  write ( *, '(a)'       ) '  Standard rule:'
  write ( *, '(a)'       ) '    Integral ( R(1) <= x <= R(2) ) f(x) dx'
  ...
  do i = 1, 2
    write ( *, '(5g24.16)' ) r(1:dim_num,i)
  end do
!
!  Rescale the weights, and translate the abscissas.
!
  volume = abs ( product ( r(1:dim_num,2) - r(1:dim_num,1) ) )
  w(1:order) = w(1:order) / volume

  do dim = 1, dim_num
    x(dim,1:order) = ( x(dim,1:order) - r(dim,1) ) &
                   / ( r(dim,2) - r(dim,1) )
  end do
!
...
!
  ...
  do degree = 0, degree_max
    ...
    do
      ...
      call monomial_quadrature ( dim_num, expon, order, w, x, &
        quad_error )

      write ( *, '(2x,f24.16,3x,i2,4x,10i2)' ) &
...
!
  ...
  value = 1.0D+00 / real ( product ( expon(1:dim_num) + 1 ), kind = 8 )

  return
end
subroutine monomial_quadrature ( dim_num, expon, order, w, x, quad_error )
...
!
!    Input, real ( kind = 8 ) X(DIM_NUM,ORDER), the quadrature points.
!
...
!
  call monomial_int01 ( dim_num, expon, exact )
!
!  Evaluate the monomial at the quadrature points.
!
  call monomial_value ( dim_num, order, x, expon, value )
!
...
subroutine monomial_value ( dim_num, order, x, expon, value )
...
!    Input, integer ( kind = 4 ) DIM_NUM, the spatial dimension.
!
!    Input, integer ( kind = 4 ) ORDER, the number of points at which the
!    monomial is to be evaluated.
!
!    Input, real ( kind = 8 ) X(DIM_NUM,ORDER), the point coordinates.
!
!    Input, integer ( kind = 4 ) EXPON(DIM_NUM), the exponents.
!
!    Output, real ( kind = 8 ) VALUE(ORDER), the value of the monomial.
!
  implicit none
