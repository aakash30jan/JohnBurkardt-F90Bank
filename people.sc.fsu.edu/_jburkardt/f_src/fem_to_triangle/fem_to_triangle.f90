...
!
  ...
  integer ( kind = 4 ) m
  real ( kind = 8 ) node_att(0,0)
  integer ( kind = 4 ) node_att_num
  integer ( kind = 4 ) node_marker(0,0)
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'FEM_TO_TRIANGLE'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Convert a 2D mesh from FEM to TRIANGLE format.'
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  Read:'
...
!  Read the node data.
!
  call r8mat_header_read ( fem_node_filename, m, node_num )
  ...
  call r8mat_data_read ( fem_node_filename, m, node_num, node_x )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read the data in "' // trim ( fem_node_filename ) //'".'
  ...
  if ( m /= 2 ) then
    write ( *, '(a)' ) ''
    write ( *, '(a)' ) 'FEM_TO_TRIANGLE - Fatal error!'
    write ( *, '(a)' ) '  Spatial dimension M is not 2.'
    stop 1
  end if
!
!  Read the element data.
!
  call i4mat_header_read ( fem_element_filename, element_order, element_num )
  ...
  call i4mat_data_read ( fem_element_filename, element_order, element_num, &
    element_node )
  ...
  if ( element_order /= 3 ) then
    write ( *, '(a)' ) ''
    write ( *, '(a)' ) 'FEM_TO_TRIANGLE - Fatal error!'
    write ( *, '(a)' ) '  FEM elements must be 3-node triangles.'
    stop 1
  end if
!
...
!
  ...
  call triangle_node_write ( triangle_node_filename, node_num, m, &
    node_att_num, node_marker_num, node_x, node_att, node_marker )

  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  Created "' // trim ( triangle_node_filename ) // '"'

  element_att_num = 0

  call triangle_element_write ( triangle_element_filename, element_num, &
    element_order, element_att_num, element_node, element_att )

  write ( *, '(a)' ) '  Created "' // trim ( triangle_element_filename ) // '"'
!
!  Free memory.
!
...
!    Input, integer ( kind = 4 ) ELEMENT_ORDER, the order of the elements.
!
!    Input, integer ( kind = 4 ) ELEMENT_ATT_NUM, the number of element 
!    attributes.
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(ELEMENT_ORDER,ELEMENT_NUM), 
!    the indices of the nodes that make up each element.
!
!    Input, real ( kind = 8 ) ELEMENT_ATT(ELEMENT_ATT_NUM,ELEMENT_NUM), 
...
!    attributes.
!
!    Output, integer ( kind = 4 ) NODE_MARKER(NODE_MARKER_NUM,NODE_NUM), 
!    the node markers.
!
  implicit none
