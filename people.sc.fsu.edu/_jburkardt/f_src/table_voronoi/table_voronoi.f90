...
!  Parameters:
!
!    Command line argument FILE_NAME, is the name of a file in the 
!    TABLE format containing the coordinates of a point set to be analyzed.
!
  implicit none
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TABLE_VORONOI'
  write ( *, '(a)' ) '  FORTRAN90 version:'
  write ( *, '(a)' ) '  This program is given the coordinates of a set of'
  write ( *, '(a)' ) '  points in the plane, calls GEOMPACK to determine the'
  write ( *, '(a)' ) '  Delaunay triangulation of those points, and then'
  write ( *, '(a)' ) '  digests that data to produce information defining'
  write ( *, '(a)' ) '  the Voronoi diagram.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  The input file contains the following data:'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    G_NUM:    the number of generators;'
  write ( *, '(a)' ) '    G_XY:     the (X,Y) coordinates of the generators.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  The computed Voronoi information includes:'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    G_DEGREE: the degree of each Voronoi cell;'
  write ( *, '(a)' ) '    G_START:  the index of the first Voronoi vertex;'
  write ( *, '(a)' ) '    G_FACE:   the list of all Voronoi vertices;'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    V_NUM:    the number of (finite) Voronoi vertices;'
  write ( *, '(a)' ) '    V_XY:     the (X,Y) coordinates of the ' // &
                                    'Voronoi vertices;'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    I_NUM:    the number of Voronoi vertices at infinity;'
  write ( *, '(a)' ) '    I_XY:     the directions associated with the Voronoi'
  write ( *, '(a)' ) '              vertices at infinity.'
!
...
subroutine handle_file ( file_name )
...
!  Parameters:
!
!    Input, character FILE_NAME, the name of an XY file whose data
...
!    point.
!
!    Local, integer ( kind = 4 ) G_NUM, the number of generators.
!
...
!
!    Local, real ( kind = 8 ) G_XY(2,G_NUM), the point coordinates.
!
!    Local, integer ( kind = 4 ) I_NUM, the number of vertices at infinity of
!    the  Voronoi diagram.
!
...
!    diagram.
!
!    Local, real ( kind = 8 ) V_XY(2,V_NUM), the coordinates of the vertices
...
!
  ...
  real ( kind = 8 ), allocatable, dimension ( :, : ) :: g_xy
  integer ( kind = 4 ) i
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'HANDLE_FILE'
  write ( *, '(a)' ) '  Read the TABLE file "' // trim ( file_name ) // '".'

  call r8mat_header_read ( file_name, m, g_num )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  The header has been read.'
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  The spatial dimension of the data M = ', m
  write ( *, '(a,i8)' ) '  The number of generators, G_NUM = ', g_num

  if ( m < dim_num ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'HANDLE - Fatal error!'
    write ( *, '(a)' ) '  The input spatial dimension is less than 2.'
    stop
  else if ( dim_num < m ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'HANDLE - Warning:'
    write ( *, '(a)' ) '  The input spatial dimension is greater than 2.'
    write ( *, '(a)' ) '  Only the first two coordinates will be considered.'
  end if

  allocate ( i_xy(dim_num,g_num) )
  allocate ( g_degree(g_num) )
  allocate ( g_face(6*g_num) )
  allocate ( g_start(g_num) )
  allocate ( g_xy(dim_num,g_num) )
  allocate ( v_xy(dim_num,2*g_num) )

  call r8mat_data_read ( file_name, dim_num, g_num, g_xy )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  The data has been read.'

  call r8mat_transpose_print ( dim_num, g_num, g_xy, '  The generators' )

  call voronoi_data ( g_num, g_xy, g_degree, g_start, g_face, v_num, v_xy, &
    i_num, i_xy )

  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  G_START: The index of the first Voronoi vertex'
  write ( *, '(a,i8)' ) '  G_FACE: The Voronoi vertices'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '   G  G_START  G_FACE'
  do j = 1, g_num

    k = g_start(j)
    write ( *, '(a)' ) ' '
    write ( *, '(i8,5x,i8,2x,i8)' ) j, k, g_face(k)
    do i = 2, g_degree(j)
      k = k + 1
      write ( *, '(8x,5x,8x,2x,i8)' ) g_face(k)
    end do

  end do

  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  V_NUM: Number of Voronoi vertices = ', v_num

  call r8mat_transpose_print ( dim_num, v_num, v_xy, '  Voronoi vertices:' )

  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) &
    '  I_NUM: Number of Voronoi vertices at infinity = ', i_num

  call r8mat_transpose_print ( dim_num, i_num, i_xy, &
...
!
!    Input, character ( kind = 4 ) ( len = * ) TITLE, an optional title.
!
  implicit none
...
!    of the Voronoi diagram.
!
!    Output, integer ( kind = 4 ) I_NUM, the number of vertices at infinity 
!    of the Voronoi diagram.
!
