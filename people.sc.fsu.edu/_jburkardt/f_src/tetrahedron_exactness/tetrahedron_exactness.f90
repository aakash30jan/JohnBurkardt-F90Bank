...
!
  ...
  real ( kind = 8 ), allocatable, dimension ( :, : ) :: x_ref

  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TETRAHEDRON_EXACTNESS'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the polynomial exactness of a quadrature'
  write ( *, '(a)' ) '  rule for a tetrahedron by integrating all monomials'
  write ( *, '(a)' ) '  of a given degree.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  The rule will be adjusted to the unit tetrahedron.'
!
...
!
  ...
  if ( dim_num /= 3 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TETRAHEDRON_EXACTNESS - Fatal error!'
    write ( *, '(a)' ) '  The quadrature abscissas must be 3 dimensional.'
    stop 1
  end if
...
!
  ...
  call r8mat_data_read ( quad_w_filename, 1, point_num, w )
!
!  Read the R file.
!
  call r8mat_header_read ( quad_r_filename, dim_num3, point_num3 )

  if ( dim_num3 /= dim_num ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TETRAHEDRON_EXACTNESS - Fatal error!'
  ...
  if ( point_num3 /= 4 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TETRAHEDRON_EXACTNESS - Fatal error!'
    write ( *, '(a)' ) '  The quadrature region file should have 4 lines.'
    stop 1
  end if

  allocate ( r(dim_num,4) )

  call r8mat_data_read ( quad_r_filename, dim_num, 4, r )
!
!  Rescale the weights.
!
  call tetrahedron_volume ( r, volume )

  w(1:point_num) = ( 1.0D+00 / 6.0D+00 ) * w(1:point_num) / volume
!
!  Translate the abscissas from the reference tetrahedron to 
!  the unit tetrahedron.
!
  allocate ( x_ref(dim_num,point_num) )

  call tetrahedron_order4_physical_to_reference ( r, point_num, x, x_ref )
!
...
!
  ...
  do degree = 0, degree_max
    ...
    do
      ...
      call tet01_monomial_quadrature ( dim_num, expon, point_num, x_ref, w, &
        quad_error )
...
subroutine tet01_monomial_integral ( dim_num, expon, value )
...
subroutine tet01_monomial_quadrature ( dim_num, expon, point_num, x, w, &
  quad_error )
...
!
  call tet01_monomial_integral ( dim_num, expon, scale )
!
...
subroutine tetrahedron_order4_physical_to_reference ( tetra, n, phy, ref )

!*****************************************************************************80
!
!! TETRAHEDRON_ORDER4_PHYSICAL_TO_REFERENCE maps physical to reference points.
!
!  Discussion:
!
...
!  Parameters:
!
!    Input, real ( kind = 8 ) TETRA(3,4), the coordinates of the vertices.
...
!  Set up the matrix.
!
  a(1:3,1) = tetra(1:3,2) - tetra(1:3,1)
  a(1:3,2) = tetra(1:3,3) - tetra(1:3,1)
  a(1:3,3) = tetra(1:3,4) - tetra(1:3,1)
!
