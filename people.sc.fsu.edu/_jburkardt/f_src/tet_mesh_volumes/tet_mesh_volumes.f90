...
!
  ...
  real ( kind = 8 ) volume_ave
  character ( len = 255 ) :: volume_filename = ' '
  real ( kind = 8 ) volume_max
  real ( kind = 8 ) volume_min
  real ( kind = 8 ) volume_tot
  real ( kind = 8 ) volume_var

  write ( *, '(a)' ) ' '
  call timestamp ( )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TET_MESH_VOLUMES:'
  write ( *, '(a)' ) '  FORTRAN90 version:'
  write ( *, '(a)' ) '  Compute volume of each tetrahedron in a tet mesh.'
!
...
!
  ...
  volume_filename = trim ( prefix ) // '_volumes.txt'
!
!  Read the node data.
!
  ...
  if ( dim_num /= 3 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TET_MESH_VOLUMES - Fatal error!'
...
!
  ...
  do element = 1, element_num
    ...
    call tetrahedron_volume ( tetra, volume(element) )
  end do

  volume_max = maxval ( volume(1:element_num) )
  volume_min = minval ( volume(1:element_num) )
  volume_ave = sum ( volume(1:element_num) ) / real ( element_num, kind = 8 )
  volume_tot = sum ( volume(1:element_num) )
  call r8vec_variance ( element_num, volume, volume_var )
 
  write ( *, '(a)' ) ' '
  write ( *, '(a,g14.6)' ) '  Minimum: ', volume_min
  write ( *, '(a,g14.6)' ) '  Average: ', volume_ave
  write ( *, '(a,g14.6)' ) '  Maximum: ', volume_max
  write ( *, '(a,g14.6)' ) '  Total:   ', volume_tot
  write ( *, '(a,g14.6)' ) '  Variance:', volume_var

  call r8mat_write ( volume_filename, 1, element_num, volume )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) &
    '  Full list of volumes written to "' // trim ( volume_filename ) // '".'
!
!  Free memory.
!
