...
!
  ...
  w(1:n) = r8_pi / real ( n, kind = 8 )

  do i = 1, n
    x(i) = cos ( r8_pi * real ( 2 * n + 1 - 2 * i, kind = 8 ) &
                       / real ( 2 * n, kind = 8 ) )
  end do
...
!
  ...
  do i = 1, n
    angle = r8_pi * real ( n + 1 - i, kind = 8 ) / real ( n + 1, kind = 8 )
    w(i) = r8_pi / real ( n + 1, kind = 8 ) * ( sin ( angle ) )**2
    x(i) = cos ( angle )
  end do
...
!
  ...
  do i = 1, n
    ...
    angle = real ( n - i, kind = 8 ) * r8_pi &
          / real ( n - 1, kind = 8 )
    x(i) = cos ( angle )

  end do

  w(1)     = r8_pi / real ( 2 * ( n - 1 ), kind = 8 )
  w(2:n-1) = r8_pi / real (       n - 1,   kind = 8 )
  w(n)     = r8_pi / real ( 2 * ( n - 1 ), kind = 8 )
...
!
  ...
  do i = 1, n
    theta(i) = real ( 2 * ( n + 1 - i ) - 1, kind = 8 ) * r8_pi &
             / real ( 2 * n,     kind = 8 )
  end do
...
!
  ...
  do i = 1, n
    theta(i) = real ( n + 1 - i, kind = 8 ) * r8_pi &
             / real ( n + 1, kind = 8 )
  end do
...
!
  ...
  call cgqf ( n, kind, alpha, beta, a, b, x, w )
...
!
  ...
  cc = sqrt ( r8_pi ) * r8_gamma ( real ( n, kind = 8 ) ) &
    / ( 2.0D+00 ** ( n - 1 ) )
...
!
  do i = 1, n
    x(i) = - cos ( 2.0D+00 * r8_pi * real (     i - 1, kind = 8 ) &
                                   / real ( 2 * n - 1, kind = 8 ) )
  end do
