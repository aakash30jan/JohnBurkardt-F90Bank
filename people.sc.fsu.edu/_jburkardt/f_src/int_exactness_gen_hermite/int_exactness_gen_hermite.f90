...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS_GEN_HERMITE'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the polynomial exactness of a'
  write ( *, '(a)' ) '  generalized Gauss-Hermite quadrature rule'
  write ( *, '(a)' ) '  by integrating exponentially weighted monomials'
  write ( *, '(a)' ) '  up to a given degree over the (-oo,oo) interval.'
!
...
!
  if ( 4 <= arg_num ) then
    ...
    write ( *, '(a)' ) '  0: standard generalized rule for integrating ' // &
      '|x|^alpha * exp(-x^2) * f(x);'
    write ( *, '(a)' ) '  1: modified generalized rule for integrating ' // &
      '                        f(x).'
    write ( *, '(a)' ) ' '
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS_GEN_HERMITE: User input:'
...
!
  ...
  if ( dim_num /= 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'INT_EXACTNESS_GEN_HERMITE - Fatal error!'
...
!  Discussion:
!
!    H(n,alpha) = Integral ( -oo < x < +oo ) x^n |x|^alpha exp(-x*x) dx
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!
  call gen_hermite_integral ( expon, alpha, exact )
!
