...
!
  ...
  if ( dim_num2 /= 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'HYPERBALL_VOLUME_QUAD - Fatal error!'
...
!
  ...
  call hyperball01_indicator ( dim_num, point_num, x, fx )

  estimate = dot_product ( weight, fx )
  call hyperball01_volume ( dim_num, exact )
...
subroutine hyperball01_indicator ( dim_num, point_num, x, fx )
...
!
!    Output, real ( kind = 8 ) FX(POINT_NUM), the indicator value.
!
  implicit none
...
subroutine hyperball01_volume ( dim_num, volume )
...
!
  ...
  if ( mod ( dim_num, 2 ) == 0 ) then
    m = dim_num / 2
    do i = 1, m
      volume = volume * r8_pi / real ( i, kind = 8 )
    end do
  else
    m = ( dim_num - 1 ) / 2
    do i = 1, m
      volume = volume * r8_pi * 2.0D+00
    end do
    do i = m+1, 2*m+1
