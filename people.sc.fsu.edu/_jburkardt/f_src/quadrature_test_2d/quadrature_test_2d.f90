...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'QUADRATURE_TEST_2D'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the accuracy of a 2D quadrature'
  write ( *, '(a)' ) '  rule by integrating test functions.'
!
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'QUADRATURE_TEST_2D: User input:'
...
!
  ...
  if ( dim_num /= 2 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'QUADRATURE_TEST_2D - Fatal error!'
    write ( *, '(a,i8)' ) &
      '  The X file seems to have spatial dimension = ', dim_num
    write ( *, '(a)' ) '  This program requires a spatial dimension of 2.'
    stop
  end if

  allocate ( x(dim_num,point_num) )

  call dtable_data_read ( quad_x_filename, dim_num, point_num, x )
!
!  Read the W file.
!
  call dtable_header_read ( quad_w_filename, dim_num2, point_num2 )

  if ( dim_num2 /= 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'QUADRATURE_TEST - Fatal error!'
...
!  Read the R file.
!
  call dtable_header_read ( quad_r_filename, dim_num2, point_num2 )
...
!
  ...
  do problem = 1, problem_num
    ...
    volume2 = product ( b(1:dim_num) - a(1:dim_num) )
!
...
!
      call p00_fun ( problem, point_num, x2, f )
!
