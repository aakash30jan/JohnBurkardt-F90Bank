...
!  Discussion:
!
!    SPARSE_GRID_MIXED_SIZE_PRB tests the SPARSE_GRID_MIXED_SIZE routines.
!
!  Licensing:
!
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'SPARSE_GRID_MIXED_SIZE_PRB'
  write ( *, '(a)' ) '  FORTRAN90 version'
!
!  Using a tolerance that is less than 0.0 means that there will be no
!  consolidation of duplicate points.
!
  tol = - 1.0D+00
  call sparse_grid_mixed_size_tests ( tol )
!
...
subroutine sparse_grid_mixed_size_tests ( tol )
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'SPARSE_GRID_MIXED_SIZE_TESTS'
  write ( *, '(a)' ) '  Call SPARSE_GRID_MIXED_SIZE_TEST with various arguments.'
  ...
  call sparse_grid_mixed_size_test ( dim_num, level_max_min, level_max_max, rule, &
    alpha, beta, tol )
  deallocate ( alpha )
  deallocate ( beta )
  deallocate ( rule )
...
subroutine sparse_grid_mixed_size_test ( dim_num, level_max_min, &
  level_max_max, rule, alpha, beta, tol )

!****************************************************************************80
!
!! SPARSE_GRID_MIXED_SIZE_TEST tests SPARSE_GRID_MIXED_SIZE.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'SPARSE_GRID_MIXED_SIZE_TEST'
  write ( *, '(a)' ) '  SPARSE_GRID_MIXED_SIZE returns the number of distinct'
  write ( *, '(a)' ) '  points in a multidimensional sparse grid with mixed factors.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  SPARSE_GRID_MIXED_SIZE_TOTAL returns the TOTAL number of'
  write ( *, '(a)' ) '  points in a multidimensional sparse grid with mixed factors,'
  write ( *, '(a)' ) '  without checking for duplication.'
  write ( *, '(a)' ) ' '
  ...
  do dim = 1, dim_num
    if ( rule(dim) ==  1 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'SPARSE_GRID_MIXED_SIZE_TEST - Fatal error!'
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) ' LEVEL_MIN LEVEL_MAX POINT_NUM POINT_NUM'
