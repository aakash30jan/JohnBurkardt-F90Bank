...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N_MAX, the maximum value of N to be considered.
!
!    Input, integer ( kind = 4 ) K_MAX, the maximum value of K to be considered.
!
!    Input, integer ( kind = 4 ) N, the size of the family.
!
!    Input, integer ( kind = 4 ) K, the size of the object.
!
!    Input, integer ( kind = 4 ) FAMILY, specifies the combinatorial family.
...
!
!    Input, integer ( kind = 4 ) MGO, is
!    1 to request B(N1,K1), or
!    2 to request B(N2,K2).
!
!    Input, integer ( kind = 4 ) B(N_MAX,K_MAX), the precomputed matrix 
!    enumerating the combinatorial family.
!
!    Output, integer ( kind = 4 ) BNEW, the value of B(N1,K1) or B(N2,K2).
!
  implicit none
...
!    Input, integer ( kind = 4 ) K, the size of the object.
!
!    Input, integer ( kind = 4 ) MU(M+1), part of the code for the object.
!
!    Input, integer ( kind = 4 ) NU(M+1), part of the code for the object.
!
!    Input, integer ( kind = 4 ) EDGE(M), part of the code for the object.
!
!    Input, integer ( kind = 4 ) M, the size of the EDGE, MU and NU arrays.
!
!    Output, integer ( kind = 4 ) COMPOSITION(K), records the composition.
...
!    Input, integer ( kind = 4 ) M, the size of the EDGE, MU and NU arrays.
!
!    Output, integer ( kind = 4 ) PARTITION_NUM(M), records the integers
...
!    Input, integer ( kind = 4 ) M, the size of the EDGE, MU and NU arrays.
!
!    Output, integer ( kind = 4 ) PARTITION_SET(N), records, for each item, the
...
!    Input, integer ( kind = 4 ) M, the size of the EDGE, MU and NU arrays.
!
!    Output, integer ( kind = 4 ) PERM_CYCLE(N), records, for each item,
...
!    Input, integer ( kind = 4 ) M, the size of the EDGE, MU and NU arrays.
!
!    Output, integer ( kind = 4 ) PERM(N); PERM(I) is the image of item I
...
!    Input, integer ( kind = 4 ) NU(M+1), part of the code for the object.
!
!    Input, integer ( kind = 4 ) EDGE(M), is not used for this routine.
!
!    Input, integer ( kind = 4 ) M, determines the size of the EDGE, MU 
!    and NU arrays.
!
!    Output, integer ( kind = 4 ) SUBSET(K), the indices of the items in 
!    the subset.
!
  implicit none
...
!    Input, integer ( kind = 4 ) M, the size of the EDGE, MU and NU arrays.
!
!    Output, integer ( kind = 4 ) SUBSPACE(K,N), contains K rows which
...
!
!    Input, integer ( kind = 4 ) K, the number of parts.
!
!    Input, integer ( kind = 4 ) COMPOSITION(K), the parts of the composition.
!
...
!    7, Compositions of N into K parts.
!
!    Input, integer ( kind = 4 ) N_MAX, the maximum size of N to be considered.
!
!    Input, integer ( kind = 4 ) K_MAX, the maximum size of K to be considered.
!
!    Output, integer ( kind = 4 ) B(N_MAX,K_MAX), the table.
!
  implicit none
...
!    Input, integer ( kind = 4 ) K_MAX, the maximum size of K to be considered.
!
!    Input, integer ( kind = 4 ) B(N_MAX,K_MAX), the enumeration table for the
!    family.  The user must evaluate this table by calling FAMILY_ENUMERATE.
!
!    Input, integer ( kind = 4 ) N, the size of the family.
!    1 <= N <= N_MAX.
!
...
!
!    Input/output, integer ( kind = 4 ) RANK, the rank of an object.
!
!    Input/output, integer ( kind = 4 ) M, the length of a code.
!
!    Input/output, integer ( kind = 4 ) MU(M+1), part of a code.
!
!    Input/output, integer ( kind = 4 ) NU(M+1), part of a code.
!
!    Input/output, integer ( kind = 4 ) EDGE(M), part of a code.
!
  implicit none
...
!    1 <= K <= K_MAX.
!
!    Input, integer ( kind = 4 ) M, the length of the code.  For
...
!    Input, integer ( kind = 4 ) EDGE(M), part of the code for the object.
!
!    Output, integer ( kind = 4 ) RANK, the rank of the object.
!
  implicit none
...
!    Output, integer ( kind = 4 ) RANK, the rank of the object.
!
!    Output, integer ( kind = 4 ) M, the length of the code.  For
!    families 1 through 5, M is always equal to N.
!    For family 6, M varies, but is never greater than N.
!    For family 7, M = N+K-1.
!
!    Output, integer ( kind = 4 ) MU(M+1), part of the code for the object.
!
!    Output, integer ( kind = 4 ) NU(M+1), part of the code for the object.
!
!    Output, integer ( kind = 4 ) EDGE(M), part of the code for the object.
!
  implicit none
...
!    1 <= K <= K_MAX.
!
!    Input, integer ( kind = 4 ) RANK, the rank of the object.
!
!    Output, integer ( kind = 4 ) M, the length of the code.  For
!    families 1 through 5, M is always equal to N.
!    For family 6, M varies, but is never greater than N.
...
!    Input, integer ( kind = 4 ) N, the size of the set.
!
!    Input, integer ( kind = 4 ) K, the size of the largest part.
!
!    Input, integer ( kind = 4 ) M, the number of parts.
!
!    Input, integer ( kind = 4 ) PARTITION_NUM(M) contains the (positive)
!    summands.
!
...
!    Input, integer ( kind = 4 ) K, the number of parts.
!
!    Input, integer ( kind = 4 ) PARTITION_SET(N), records, for each partition,
...
!    Input, integer ( kind = 4 ) K, the number of parts.
!
!    Input, integer ( kind = 4 ) PERM_CYCLE(N), lists the elements being 
!    permuted, with the first element of a cycle negative, and the successive
...
!    7, Compositions of N into K parts.
!
!    Output, integer ( kind = 4 ) PHI, the value of the PHI coefficient
!    in the recurrence.
!
  implicit none
...
!    7, Compositions of N into K parts.
!
!    Output, integer ( kind = 4 ) PSI, the value of the PSI coefficient
!    in the recurrence.
!
  implicit none
...
!    7, Compositions of N into K parts.
!
!    Input, integer ( kind = 4 ) TASK, specifies the task.
...
!    1 <= K <= K_MAX otherwise.
!
!    Input/output, integer ( kind = 4 ) MU(M+1), part of the code for the 
!    object.  The use and meaning of MU depend on TASK and FAMILY.
!
!    Input/output, integer ( kind = 4 ) NU(M+1), part of the code for the
!    object.  The use and meaning of NU depend on TASK and FAMILY.
!
!    Input/output, integer ( kind = 4 ) EDGE(M), part of the code for the 
!    object.  The use and meaning of EDGE depend on TASK and FAMILY.
!
!    Input/output, integer ( kind = 4 ) M, the length of the code.  For
!    families 1 through 5, M is always equal to N.
!    For family 6, M varies, but is never greater than N.
!    For family 7, M = N+K-1.
!
...
!
!    Input/output, integer ( kind = 4 ) SEED, a seed for the random 
!    number generator.  SEED is only needed for TASK 4.
!
!    Output, integer ( kind = 4 ) B(N_MAX,K_MAX), is computed by the program 
!    as needed.  B(I,J) is the number of objects of "size" J in the family of 
!    size I.  The user may examine B, but should not change its entries 
!    or dimension.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) K, the size of the object.  K = 0 is allowed.
!
!    Input, integer ( kind = 4 ) SUBSET(K), the indices of the items in 
!    the subset.
!
...
!    7, Compositions of N into K parts.
!
!    Input, integer ( kind = 4 ) MGO, is 1 to request index K1, or 2 to request
!    index K2.
!
!    Output, integer ( kind = 4 ) XNEW, the value of the requested index.
!
  implicit none
