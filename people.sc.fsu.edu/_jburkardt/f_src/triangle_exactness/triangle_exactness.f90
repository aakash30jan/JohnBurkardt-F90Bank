...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TRIANGLE_EXACTNESS'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Investigate the polynomial exactness of a quadrature'
  write ( *, '(a)' ) '  rule for the triangle by integrating all monomials'
  write ( *, '(a)' ) '  up to a given degree.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  The rule will be adjusted to the unit triangle.'
!
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TRIANGLE_EXACTNESS: User input:'
...
!
  ...
  if ( dim_num /= 2 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGLE_EXACTNESS - Fatal error!'
    write ( *, '(a)' ) '  The quadrature abscissas must be two dimensional.'
    stop 1
  end if
...
!
  ...
  if ( point_num3 /= 3 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGLE_EXACTNESS - Fatal error!'
    write ( *, '(a)' ) '  The quadrature region file should have three lines.'
    stop 1
  end if

  allocate ( r(dim_num,3) )

  call r8mat_data_read ( quad_r_filename, dim_num, 3, r )
!
!  Rescale the weights.
!
  call triangle_area ( r, area )

  w(1:point_num) = 0.5D+00 * w(1:point_num) / area
!
!  Translate the abscissas from the reference triangle to the unit triangle.
!
  allocate ( x_ref(dim_num,point_num) )

  call triangle_order3_physical_to_reference ( r, point_num, x, x_ref )
!
...
!
  ...
  do degree = 0, degree_max
    ...
    do
      ...
      call triangle01_monomial_quadrature ( dim_num, expon, point_num, x_ref, &
        w, quad_error )
...
subroutine triangle01_monomial_integral ( dim_num, expon, value )

!*****************************************************************************80
!
!! TRIANGLE01_MONOMIAL_INTEGRAL: integral of a monomial over the unit triangle.
!
!  Discussion:
!
...
subroutine triangle01_monomial_quadrature ( dim_num, expon, point_num, x, w, &
  quad_error )

!*****************************************************************************80
!
!! TRIANGLE01_MONOMIAL_QUADRATURE: approximate monomial integral, unit triangle.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!
  call triangle01_monomial_integral ( dim_num, expon, scale )
!
...
!
  area = 0.5D+00
  quad = area * dot_product ( w, value ) / scale
!
!  Error:
!
  exact = 1.0D+00
