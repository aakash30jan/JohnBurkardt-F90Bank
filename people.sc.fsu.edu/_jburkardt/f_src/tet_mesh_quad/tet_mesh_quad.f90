...
!
!    Local, real ( kind = 8 ) NODE_XYZ(DIM_NUM,NODE_NUM), the point set.
!
!    Local, real ( kind = 8 ) VALUE(VALUE_DIM,VALUE_NUM), the VALUE_DIM
!    components of a vector function evaluated at each of the NODE_NUM nodes.
!
!    Local, integer ( kind = 4 ) VALUE_DIM, the number of components of the
!    function.
!
!    Local, integer ( kind = 4 ) VALUE_NUM, the number of points at which
!    function values were supplied.  This must be equal to NODE_NUM.
!
  implicit none
  ...
  real ( kind = 8 ), allocatable :: quad(:)
  integer ( kind = 4 ) value_dim
  character ( len = 255 ) value_filename
...
!
  ...
  value_filename = trim ( prefix ) // '_values.txt'
!
!  Read the node data.
!
  ...
  if ( node_dim /= 3 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TET_MESH_QUAD - Fatal error!'
    write ( *, '(a)' ) '  Dataset must have spatial dimension 3.'
    stop
  end if

  allocate ( node_xyz(1:node_dim,1:node_num) )

  call r8mat_data_read ( node_filename, node_dim, node_num, node_xyz )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read the data in "' &
    // trim ( node_filename ) //'".'

  call r8mat_transpose_print_some ( node_dim, node_num, node_xyz, 1, 1, &
    node_dim, 5, '  First 5 nodes:' )
!
...
!
  ...
  if ( element_order /= 4 .and. element_order /= 10 ) then
    write ( *, * ) ' '
    write ( *, '(a)' ) 'TET_MESH_QUAD - Fatal error!'
    write ( *, '(a)' ) '  Data is not for a 4-node or 10-node tet mesh.'
    stop
  end if
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Tetrahedron order = ', element_order
...
!  Read the value data.
!
  call r8mat_header_read ( value_filename, value_dim, value_num )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read the header of "' &
    // trim ( value_filename ) //'".'
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Value dimension VALUE_DIM =   ', value_dim
  write ( *, '(a,i8)' ) '  Number of values VALUE_NUM  = ', value_num

  if ( value_num /= node_num ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TET_MESH_QUAD - Fatal error!'
    write ( *, '(a)' ) '  Number of values must equal number of nodes.'
    stop
  end if

  allocate ( value(1:value_dim,1:value_num) )

  call r8mat_data_read ( value_filename, value_dim, value_num, value )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read the data in "' &
    // trim ( value_filename ) //'".'

  call r8mat_transpose_print_some ( value_dim, value_num, value, 1, 1, &
...
!
  allocate ( volume(1:element_num) )

  volume(1:element_num) = ( 1.0D+00 / 6.0D+00 ) * abs ( &
      ( node_xyz(1,element_node(2,1:element_num)) &
      - node_xyz(1,element_node(1,1:element_num)) )       &
      * ( ( node_xyz(2,element_node(3,1:element_num)) &
        - node_xyz(2,element_node(1,1:element_num)) )   &
        * ( node_xyz(3,element_node(4,1:element_num)) &
        - node_xyz(3,element_node(1,1:element_num)) )   &
        - ( node_xyz(3,element_node(3,1:element_num)) &
        - node_xyz(3,element_node(1,1:element_num)) )   &
        * ( node_xyz(2,element_node(4,1:element_num)) &
        - node_xyz(2,element_node(1,1:element_num)) ) ) &
    + ( node_xyz(2,element_node(2,1:element_num)) &
        - node_xyz(2,element_node(1,1:element_num)) )       &
      * ( ( node_xyz(3,element_node(3,1:element_num)) &
        - node_xyz(3,element_node(1,1:element_num)) )   & 
        * ( node_xyz(1,element_node(4,1:element_num)) &
        - node_xyz(1,element_node(1,1:element_num)) )   &
        - ( node_xyz(1,element_node(3,1:element_num)) &
        - node_xyz(1,element_node(1,1:element_num)) )   &
        * ( node_xyz(3,element_node(4,1:element_num)) &
        - node_xyz(3,element_node(1,1:element_num)) ) ) &
    + ( node_xyz(3,element_node(2,1:element_num)) &
      - node_xyz(3,element_node(1,1:element_num)) )       &
      * ( ( node_xyz(1,element_node(3,1:element_num)) &
        - node_xyz(1,element_node(1,1:element_num)) )   &
        * ( node_xyz(2,element_node(4,1:element_num)) &
...
!
  allocate ( quad(1:value_dim) )

  quad(1:value_dim) = matmul ( &
    ...
    + value(1:value_dim,element_node(4,1:element_num)) ), &
    volume(1:element_num) ) / 4.0D+00

  write ( *, '(a)' ) ' '
  write ( *, '(a,g14.6)' ) &
    '  Tetrahedral mesh volume is ', sum ( volume(1:element_num) )

  call r8vec_print ( value_dim, quad, '  Integral estimates:' )
!
!  Free memory.
!
