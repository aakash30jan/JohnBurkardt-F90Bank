...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'WEDGE_EXACTNESS'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Investigate the polynomial exactness of'
  write ( *, '(a)' ) '  a quadrature rule for the unit wedge.'
!
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'WEDGE_EXACTNESS: User input:'
  write ( *, '(a)' ) &
    '  Quadrature rule W file = "' // trim ( quad_w_filename ) // '".'
  write ( *, '(a)' ) &
    '  Quadrature rule X file = "' // trim ( quad_x_filename ) // '".'
...
!
  ...
  if ( dim_num /= 3 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'WEDGE_EXACTNESS - Fatal error!'
    write ( *, '(a)' ) '  Spatial dimension must be 3.'
    stop
  end if

  allocate ( x(dim_num,order) )

  call r8mat_data_read ( quad_x_filename, dim_num, order, x )
!
!  Read the W file.
!
  call r8mat_header_read ( quad_w_filename, dim_num2, order2 )
...
!
  ...
  do degree = 0, degree_max
    ...
    do
      ...
      quad = wedge01_volume ( ) * dot_product ( w(1:order), v(1:order) )
...
!*****************************************************************************80
!
!! WEDGE01_VOLUME: volume of a unit wedge.
!
!  Discussion:
!
!    The integration region is:
