...
!  Discussion:
!
!    This program investigates a standard Gauss-Chebyshev type 1 quadrature 
!    rule by using it to integrate monomials over [-1,+1], and comparing the
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS_CHEBYSHEV1'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) &
    '  Investigate the polynomial exactness of a Gauss-Chebyshev'
  write ( *, '(a)' ) '  type 1 quadrature rule by integrating weighted '
  write ( *, '(a)' ) &
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS_CHEBYSHEV1: User input:'
...
!
  ...
  if ( dim_num /= 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'INT_EXACTNESS_CHEBYSHEV1 - Fatal error!'
...
!
  ...
  call r8mat_data_read ( quad_w_filename, dim_num, order, w )
!
!  Read the R file.
!
  ...
  call r8mat_data_read ( quad_r_filename, dim_num, point_num2, r )
!
!  Print the input quadrature rule.
!
  write ( *, '(a)'       ) ' '
  write ( *, '(a)'       ) '  The quadrature rule to be tested is'
  write ( *, '(a)'       ) '  a Gauss-Chebyshev type 1 rule'
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  A Gauss-Chebyshev type 1 rule would be able to exactly'
  write ( *, '(a,i8)' ) &
  ...
  do degree = 0, degree_max
    ...
    call monomial_quadrature_chebyshev1 ( expon, order, w, x, quad_error )
...
subroutine monomial_quadrature_chebyshev1 ( expon, order, w, x, quad_error )

!*****************************************************************************80
!
!! MONOMIAL_QUADRATURE_CHEBYSHEV1 approximates a Chebyshev 1 monomial integral.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!  Get the exact value of the integral.
!
  call chebyshev1_integral ( expon, exact )
!
