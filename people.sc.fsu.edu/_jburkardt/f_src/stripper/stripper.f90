...
!
  ...
  integer ( kind = 4 ), parameter :: mmax = 5000
...
!
!    Input, integer ( kind = 4 ) LUNIT1, is the FORTRAN unit number of the file
!    from which input is read, unless the input file is '*'.
!
!    Input, integer ( kind = 4 ) MRECI, is the maximum number of input records
!    that may be read.
!
...
!
!    Input, integer ( kind = 4 ) LUNIT, the logical unit from which input is 
!    to be read.
!
...
!
  ...
  write ( *, '(a)' ) '  Set LWRAP to MMAX ( = 5000).'
...
!
  ...
  write ( *, '(a)' ) &
    'ICapF =    -1 lower, +1 upper, +2 first upper language text;'

  read ( *, * )

  write ( *, '(a)' ) &
    'ICapFC =   -1 lower, +1 upper, +2 first upper language comments;'
...
!
!    Input, integer ( kind = 4 ) IREPH, the number of horizontal repetitions 
!    of each line.
!
!    Input, integer ( kind = 4 ) IREPV, the number of vertical repetitions 
!    of each line.
!
!    ???, integer ISAY, ???
!
...
!
!    Output, integer ( kind = 4 ) NUMCON is the number of control characters 
!    in LINE.
!
  implicit none
...
!
!    Output, integer ( kind = 4 ) ILO is the location of the first character of 
...
!
!    Output, integer ( kind = 4 ) IHI is the index of the last character of 
!    the INDX-th word, or 0 if there was no INDX-th word.
!
  implicit none
