...
!  Discussion:
!
!    TOMES672_PRB tests the TOMS672 library.
!
...
!
!    Input, integer ( kind = 4 ) I, the current stage of the set of nested
!    quadrature rule.
!
!    Input, integer ( kind = 4 ) IDEG, the expected polynomial accuracy of 
!    the quadrature rule.
!
...
!
!    Input, integer ( kind = 4 ) IWORK(max(M,N)), convergence flags.  Elements 
...
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of WORKA in the
!    calling program.
!
...
!
!    Input, integer ( kind = 4 ) N, the number of nodes in the current 
!    quadrature rule.
!
!    Input, integer ( kind = 4 ) NODES, the number of extended nodes found.
...
!
!    Input, integer ( kind = 4 ) NTEST, the number of tests made of the 
!    quadrature rule.
!
!    Input, real ( kind = 8 ) PNODES(N), the nodes of the extended quadrature 
!    rule.
!
