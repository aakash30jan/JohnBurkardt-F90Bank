...
!
  ...
  integer ( kind = 4 ) node
  integer ( kind = 4 ), allocatable, dimension ( : ) :: node_boundary
...
!
  call i4vec_write ( boundarynode_filename, node_num, node_boundary )
...
!    order.
!
!    Output, integer ( kind = 4 ) NODE_BOUNDARY(NODE_NUM), is 1 if the node
!    is on a boundary edge, 0 otherwise.
!
  implicit none
