...
!  Parameters:
!
!    Local, integer ( kind = 4 ), parameter BF_MAX, the maximum number of 
!    boundary faces.  I don't know a reasonable formula for this quantity.
...
!    records in FC.
!
!    Input/output, integer ( kind = 4 ) FC_NUM, current number of records 
!    used in FC.
!
...
!
!    Output, integer ( kind = 4 ) IN, contains following value if INTEST 
!    is .TRUE.:
...
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of array A in 
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) L, U, the lower and upper index of
!    part of heap.
!
!    Input, integer ( kind = 4 ) K, the dimension of points.
!
...
!
!    Input, integer ( kind = 4 ) SIZHT, the size of the hash table HT; a good 
!    choice is a prime number which is about 1/8 * NFACE (or 3/2 * NPT for 
!    random points from the uniform distribution).
!
!    Input, integer ( kind = 4 ) BF_MAX, the maximum size available for BF 
!    array.  This needs to be at least as big as the number of boundary faces.
!
!    Input, integer ( kind = 4 ) FC_MAX, the maximum size available for 
...
!
!    Input/output, integer ( kind = 4 ) VM(1:NPT), the vertices of VCL to be 
...
!    NPT.
!
!    Output, integer ( kind = 4 ) BF_NUM, the number of positions used in BF.
!    BF_NUM <= BF_MAX.
!
!    Output, integer ( kind = 4 ) FC_NUM, the number of positions used in FC.
!    FC_NUM <= FC_MAX.
!
!    Output, integer ( kind = 4 ) NFACE, the number of faces in triangulation; 
!    NFACE <= FC_NUM.
!
!    Output, integer ( kind = 4 ) NTETRA, the number of tetrahedra in 
!    the triangulation.
!
!    Output, integer ( kind = 4 ) BF(1:3,1:BF_NUM), boundary face records 
...
!
!    Output, integer ( kind = 4 ) FC(1:7,1:FC_NUM), face records which are in 
...
!
!    Output, integer ( kind = 4 ) HT(0:SIZHT-1), a hash table using direct
...
!
!    Input/output, integer ( kind = 4 ) MAP(1:NV), on input, contains vertex 
...
!
!    Output, integer ( kind = 4 ) I3, I4, the indices such that
...
!
!    Input/output, integer ( kind = 4 ) HT(0:P-1), the hash table using 
...
!    Input, integer ( kind = 4 ) IND, the index of FC array.
!
!    Input, integer ( kind = 4 ) A, B, C, D, E, the first 5 fields of FC 
!    record (or column).
!
!    Input, integer ( kind = 4 ) N, the upper bound on vertex indices.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) A,B,C, first 3 fields of FC record 
!    (in any order).
!
...
!
!    Input, integer ( kind = 4 ) FC(1:7,1:*), array of face records; 
!    see routine DTRIS3.
!
!    Input, integer ( kind = 4 ) HT(0:P-1), hash table using direct chaining.
!
!    Output, integer ( kind = 4 ) HTSRC, index of FC record with key A,B,C 
...
!    triangulation.
!
!    Input, integer ( kind = 4 ) NPT, the number of 3D points to be 
!    triangulated.
!
!    Input, integer ( kind = 4 ) SIZHT, the size of hash table HT.
!
!    Input/output, integer ( kind = 4 ) BF_NUM, the number of positions in BF.
!
!    Input/output, integer ( kind = 4 ) FC_NUM, the number of positions in FC.
!
!    Input, integer ( kind = 4 ) BF_MAX, the maximum size available for BF.
!
!    Input, integer ( kind = 4 ) FC_MAX, the maximum size available for FC.
!
!    Input/output, integer ( kind = 4 ) BF(1:3,1:BF_MAX), the array of 
!    boundary face records; see DTRIS3.
!
!    Input/output, integer ( kind = 4 ) FC(1:7,1:FC_MAX), the array of face 
!    records; see routine DTRIS3.
!
!    Input/output, integer ( kind = 4 ) HT(0:SIZHT-1), the hash table using 
!    direct chaining.
!
!    Input/output, integer ( kind = 4 ) NTETRA, the number of tetrahedra in 
!    triangulation.
!
!    Input/output, integer ( kind = 4 ) HDAVBF, the head pointer to available 
!    BF records.
!
!    Input/output, integer ( kind = 4 ) HDAVFC, the head pointer to available 
!    FC records.
!
!    Input, integer ( kind = 4 ) FRONT, the index of front of queue (or top 
!    of stack) of visible boundary faces.
!
!    Output, integer ( kind = 4 ) BACK, the index of back of queue (or bottom 
!    of stack) of visible boundary faces (which become interior faces).
!
!    Output, integer ( kind = 4 ) BFI, the index of FC of a boundary face 
!    containing vertex I.
!
!    Output, integer ( kind = 4 ) IERR, error flag, which is zero unless an 
!    error occurred.
...
!
!    Input, integer ( kind = 4 ) NPT, the number of 3D points to 
!    be triangulated.
!
!    Input, integer ( kind = 4 ) SIZHT, the size of hash table HT.
!
!    Input/output, integer ( kind = 4 ) FC_NUM, the number of positions used 
!    in FC array.
!
!    Input, integer ( kind = 4 ) FC_MAX, the maximum size available for FC.
!
...
!    triangulated.
!
!    Input/output, integer ( kind = 4 ) BF(1:3,1:*), the  array of boundary 
!    face records; see DTRIS3.
!
!    Input/output, integer ( kind = 4 ) FC(1:7,1:FC_MAX), the array of 
!    face records; see routine DTRIS3.
!
!    Input/output, integer ( kind = 4 ) HT(0:SIZHT-1), the hash table using 
!    direct chaining.
!
!    Input/output, integer ( kind = 4 ) NTETRA, the number of tetrahedra 
!    in triangulation.
!
!    Input/output, integer ( kind = 4 ) HDAVFC, the head pointer to available 
!    FC records.
!
!    Input/output, integer ( kind = 4 ) FRONT, BACK, the indices of front and 
!    back of queue of interior faces for which sphere test is applied.
!
!    Output, integer ( kind = 4 ) IFAC, the index of last face for which sphere
!    test applied, or 0.
!
!    Output, integer ( kind = 4 ) IERR, error flag, which is zero unless an 
!    error occurred.
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) FC_MAX, the maximum number of positions 
!    in the FC array.
!
!    Input, integer ( kind = 4 ) FC_NUM, the number of positions used in FC.
!
!    Input, integer ( kind = 4 ) VM(1:*), the indices of vertices of VCL that
!    are triangulated.
!
!    Input, integer ( kind = 4 ) FC(7,FC_MAX), array of face records; 
!    see routine DTRIS3.
!
...
!
!    Output, integer ( kind = 4 ) TETRA(4,TETRA_NUM), contains global 
...
!    Input, real ( kind = 8 ) VCL(1:3,1:*), the vertex coordinate list.
!
!    Input, integer ( kind = 4 ) VM(1:*), the indices of vertices of VCL 
!    being triangulated.
!
!    Input, integer ( kind = 4 ) BF(1:3,1:*), the array of boundary face 
!    records; see DTRIS3.
!
!    Input/output, integer ( kind = 4 ) FC(1:7,1:*), the array of face records;
...
!
!    Input/output, integer ( kind = 4 ) TOPV.  On input, index of FC of visible
!    boundary face.  On output, index of top of stack of visible boundary faces.
!
!    Input, integer ( kind = 4 ) TOPNV, the index of top of stack of boundary 
