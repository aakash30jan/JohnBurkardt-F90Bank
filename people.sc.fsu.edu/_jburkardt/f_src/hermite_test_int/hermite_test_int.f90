...
!
  ...
  if ( n < 0 ) then
    ...
    value = real ( i4_factorial2 ( n - 1 ), kind = 8 ) * sqrt ( r8_pi ) &
      / 2.0D+00 ** ( n / 2 )
...
!
!    Output, integer ( kind = 4 ) I4_FACTORIAL2, the value of the double
!    factorial.
!
  implicit none
...
!
  ...
  weight = real ( order, kind = 8 ) / sqrt ( r8_pi ) / sqrt ( 2.0D+00 )
...
!
  ...
  if ( m <= -1 ) then
    ...
    exact = real ( i4_factorial2 ( m - 1 ), kind = 8 ) * sqrt ( r8_pi ) &
      / 2.0D+00**( m / 2 )
